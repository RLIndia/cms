...
Namespace WebApplication
	Public Module NewsManager
      ...
      Dim AllNews As System.Collections.Generic.List(Of Notice) = CType(LoadObject(GetType(System.Collections.Generic.List(Of Notice)), "News"), System.Collections.Generic.List(Of Notice))
    ...
    End Function
      ...
      Public Function Compare(ByVal info1 As Notice, ByVal info2 As Notice) As Integer Implements Collections.Generic.IComparer(Of Notice).Compare
      ...
      Public Language As LanguageManager.Language = LanguageManager.Language.NotDefinite
        ...
        If (News.Language <> Find.Language) AndAlso Not News.Language = LanguageManager.Language.NotDefinite Then
          Return False
        End If
			End If
      ...
      End If
          ...
          If CBool(FindWord(News.Title, Word)) Then
            Finded = True
            Exit For
          ElseIf CBool(FindWord(News.Description, Word)) Then
            Finded = True
            Exit For
          End If
				Next
      ...
      ReadFeed(Notices, NewsRequire, "Mozilla/5.0", True)
		End Sub
		...
		Function PubDateToDate(ByVal PubDate As String) As Date
			...
			Try
        Return System.DateTime.ParseExact(PubDate, "r", Nothing).ToUniversalTime 'In google news the value is in Universal Time
        'Return System.DateTime.ParseExact(PubDate, "r", Nothing).ToUniversalTime() 
          ...
          Return System.DateTime.ParseExact(ReplaceBin(PubDate, "GMT+00:00", "GMT"), "ddd, dd MMM yyyy HH:mm:ss 'GMT'", Culture).ToUniversalTime
				Catch ex2 As Exception
					Try
            'Skype & Facebook date format
						Dim Culture As Globalization.CultureInfo = New Globalization.CultureInfo("en-US")
            Return System.DateTime.ParseExact(PubDate, "ddd, dd MMM yyyy HH:mm:ss zzz", Culture).ToUniversalTime
						'Return System.DateTime.ParseExact(PubDate.Substring(5, 20), "dd MMM yyyy HH:mm:ss", Culture)
					Catch ex3 As Exception
						Try
              Return DateSerial(CInt(Mid(PubDate, 13, 4)), Month(Mid(PubDate, 9, 3)), CInt(Mid(PubDate, 6, 2))).ToUniversalTime()
    ...
    End Function
      ...
      Public Video As String
        ...
        If CryptedUrl IsNot Nothing Then
          ...
          If Not String.IsNullOrEmpty(Me.Image) Then
            Dim Img As New WebControl(HtmlTextWriterTag.Img)
            Img.Attributes.Add("itemprop", "image")
            Img.Attributes.Add("src", Me.ImageSrc(False))
          ...
          Ctrl.Controls.Add(Components.Link(NavigateUrl, InnerText(Title), Nothing, True, Nothing, Rel.None, "name"))
          Ctrl.Controls.Add(BR)
          Dim Label As New Label
          Label.Attributes.Add("itemprop", "articleBody")
          Label.Text = HttpUtility.HtmlEncode(InnerText(Description))
      ...
      End Function
        ...
        Dim Html As String = ReadHtmlFromWeb(Me.Link, Nothing, 10000)
      ...
      Dim Added As Boolean = False
			If Not Sources Is Nothing Then
				For Each Source As NewsRequire In Sources
                    ...
                    ReDim Preserve Source.KeyWords(Source.KeyWords.Length)
			...
			If Not Added Then
				If Sources Is Nothing Then
					ReDim Sources(0)
				Else
          ReDim Preserve Sources(Sources.Length)
				End If
