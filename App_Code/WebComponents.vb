...
'Imports System.Linq 'need for c# compatibility (.Average)
Namespace WebApplication
	...
	Public Module Components
      ...
      If String.IsNullOrEmpty(Href) AndAlso Level = 0 Then
        If CurrentSetting().Aspect.MenuContractible Then
          Dim Idx As Integer = NextProgressive("idm0")
          Ctrl.ClientIDMode = UI.ClientIDMode.Static
          Ctrl.ID = "idm0" & Idx
    ...
    Function ObjectToControl(ByVal Setting As SubSite, ByVal Obj As Object, Optional TablePropertyPhraseCorrispondence As StringDictionary = Nothing, Optional ByVal TablePropertyIdPhraseCorrispondence As Collections.Generic.Dictionary(Of String, Integer) = Nothing, Optional ByVal NamePhraseBooks As String = Nothing, Optional ByVal SetByRequest As HttpRequest = Nothing, Optional ByRef BackErrorSetByRequest As Boolean = False, Optional ByVal FilterExcludeProperties() As String = Nothing, Optional ByVal FilterIncludeProperties() As String = Nothing, Optional ByVal FilterArray As Boolean = False) As Control
      ...
      NodeExplorer(Setting, TablePropertyPhraseCorrispondence, TablePropertyIdPhraseCorrispondence, NamePhraseBooks, Obj, Obj.GetType, Control, SetByRequest, BackErrorSetByRequest, 0, Obj.GetType.Name, Nothing, 0, -1, FilterExcludeProperties, FilterIncludeProperties, FilterArray)
      Dim Literal As New Literal
      Literal.Text = ControlToText(Control)
      Return Literal
    End Function

    Private Sub NodeExplorer(Setting As SubSite, TablePropertyPhraseCorrispondence As StringDictionary, TablePropertyIdPhraseCorrispondence As Collections.Generic.Dictionary(Of String, Integer), NamePhraseBooks As String, ByRef Obj As Object, ObjType As Type, Control As Control, Optional SetByRequest As HttpRequest = Nothing, Optional ByRef BackError As Boolean = False, Optional Progressive As Integer = 0, Optional Name As String = Nothing, Optional FullName As String = Nothing, Optional Level As Integer = 0, Optional IdArray As Integer = -1, Optional FilterExcludeProperties() As String = Nothing, Optional FilterIncludeProperties() As String = Nothing, Optional FilterArray As Boolean = False)
      'Exclude and Include by filter if the filter is setting
      If (FilterExcludeProperties Is Nothing OrElse Not Array.IndexOf(FilterExcludeProperties, Name) <> -1) AndAlso (FilterIncludeProperties Is Nothing OrElse Array.IndexOf(FilterIncludeProperties, Name) <> -1) Then
        ...
        Else
          ...
          If ObjType.GetMethod("ToArray") IsNot Nothing AndAlso ObjType.GetMethod("Clear") IsNot Nothing Then
            Collection = Obj
            Obj = CallByName(Obj, "ToArray", CallType.Get) 'Obj = Obj.ToArray
            ObjType = Obj.GetType
          End If
          ...
          If ObjType.IsArray Then
            If SetByRequest IsNot Nothing AndAlso SetByRequest(FullName) IsNot Nothing Then
              ...
              If Obj Is Nothing Then
                UBoundArray = -1
              Else
                UBoundArray = UBound(CType(Obj, Array))
              End If
              Dim NewUBound As Integer = CInt(SetByRequest(FullName)) - 1
              If UBoundArray <> NewUBound Then
                Try
                  Dim ReDimObj As Array = Array.CreateInstance(Type.GetType(Obj.GetType.FullName.Substring(0, Obj.GetType.FullName.Length - 2)), NewUBound + 1)
                  For N As Integer = 0 To UBound(ReDimObj)
                    If N <= UBound(CType(Obj, Array)) Then
                      ReDimObj.SetValue(CType(Obj, Array).GetValue(N), N)
                    End If
                  Next
                  Obj = ReDimObj
                  'End Select
                  ...
                  If Collection IsNot Nothing Then
                    Collection.GetType.GetMethod("Clear").Invoke(Collection, Nothing) 'Collection.Clear()                    
                  End If
                  For N As Integer = LBound(CType(Obj, Array)) To UBound(CType(Obj, Array))
                    If CType(Obj, Array).GetValue(N) Is Nothing Then
                      'Type of element in array
                      Dim TypeElement As Type = ObjType.GetElementType
                      'New istance by type
                      If TypeElement Is GetType(System.String) Then
                        CType(Obj, Array).SetValue("", N)
                      Else
                        CType(Obj, Array).SetValue(Activator.CreateInstance(TypeElement), N)
                      End If
                    End If
                    'Add all array elements to collection
                    If Collection IsNot Nothing Then
                      Dim Param(0) As Object
                      Param(0) = CType(Obj, Array).GetValue(N)
                      Collection.GetType.GetMethod("Add").Invoke(Collection, Param) 'Collection.Add(Obj(N))
                    End If
                  Next
            ...
            If Obj IsNot Nothing AndAlso FilterArray = False Then
              For Idx As Integer = LBound(CType(Obj, Array)) To UBound(CType(Obj, Array))
                If CType(Obj, Array).GetValue(Idx) IsNot Nothing Then
                  If CType(Obj, Array).GetValue(Idx).GetType.IsPrimitive OrElse CType(Obj, Array).GetValue(Idx).GetType.FullName = "System.String" Then
                    SubObjectName = CStr(Idx + 1)
                  Else
                    SubObjectName = Extension.Left(Obj.GetType.Name, Len(Obj.GetType.Name) - 2) & " " & Idx + 1
                  End If

                  SubObjectType = CType(Obj, Array).GetValue(Idx).GetType
                  Dim NewFullName As String
                  NewFullName = FullName & SubObjectName
                  Dim ObjRef As Object = CType(Obj, Array).GetValue(Idx)
                  NodeExplorer(Setting, TablePropertyPhraseCorrispondence, TablePropertyIdPhraseCorrispondence, NamePhraseBooks, ObjRef, SubObjectType, CType(Panel, Control), SetByRequest, BackError, Progressive, SubObjectName, NewFullName, Level + 1, Idx, FilterExcludeProperties, FilterIncludeProperties, FilterArray)
                  CType(Obj, Array).SetValue(ObjRef, Idx)
                End If
              Next
            End If
          ...
          Else
            ...
            If ObjType.GetMethod("GetDynamicMemberNames") IsNot Nothing AndAlso ObjType.GetMethod("GetDynamicMemberNames").ReturnType = GetType(IEnumerable(Of String)) Then
              'Is a dinamic object
              Dim Names As String() = CType(CallByName(Obj, "GetDynamicMemberNames", CallType.Get), String()) '= Obj.GetDynamicMemberNames()
              For Each SubObjectName In Names
                ...
                Else
                  NewFullName = FullName & "." & SubObjectName
                End If
                NodeExplorer(Setting, TablePropertyPhraseCorrispondence, TablePropertyIdPhraseCorrispondence, NamePhraseBooks, SubObject, SubObjectType, CType(Panel, Control), SetByRequest, BackError, Progressive, SubObjectName, NewFullName, Level + 1, -1, FilterExcludeProperties, FilterIncludeProperties, FilterArray)
                If SetByRequest IsNot Nothing Then
                  CallByName(Obj, SubObjectName, CallType.Set, SubObject)
                End If
              Next
            Else
    ...
    Private Sub ProcessValueTypeElement(ByVal Setting As SubSite, ByVal TablePropertyPhraseCorrispondence As StringDictionary, ByVal TablePropertyIdPhraseCorrispondence As Collections.Generic.Dictionary(Of String, Integer), ByVal NamePhraseBooks As String, ByRef Obj As Object, ByRef ObjType As Type, ByRef Control As Control, Optional ByVal SetByRequest As HttpRequest = Nothing, Optional ByRef BackError As Boolean = False, Optional ByVal FullName As String = Nothing)
      ...
      If TypeOf Obj Is Boolean Then
        Dim List As WebControls.ListBox = ListItem({CStr(True), CStr(False)}, CStr(Obj), {Phrase(Setting.Language, 126), Phrase(Setting.Language, 127)})
        List.ID = FullName
        List.Rows = 2
      ...
      ElseIf ObjType.IsEquivalentTo(Type.GetType("System.String")) Then
        If CBool(InStr(FullName, "color", CompareMethod.Text)) Then
          Dim Input As New WebControls.WebControl(HtmlTextWriterTag.Input)
          Input.Attributes.Add("name", FullName)
          Input.Attributes.Add("maxlength", CStr(7))
          Input.Attributes.Add("size", CStr(8))
          Input.Attributes.Add("value", CStr(Obj))
        ...
        Else
          ...
          Textarea.Controls.Add(Components.Literal(CStr(Obj)))
          ...
          Control.Controls.Add(Components.Script("checkRows(document.getElementById(""" & Textarea.ID & """))", Components.ScriptLanguage.javascript))
        End If
      Else
        Dim TextBox As New WebControls.TextBox
    ...
    Function IsColor(Text As String) As Boolean
      If Not String.IsNullOrEmpty(Text) AndAlso Text.First = "#"c Then
    ...
    End Function
      ...
      Dim Id As Integer = 0
        ...
        Dim Ctrls As Control() = Nothing
        If CBool(CtrlCollection.Count) Then
          Ctrls = CtrlCollection.ToArray
          Dim Last As HyperLink = CType(Ctrls(0), HyperLink)
      ...
      Link.Controls.Add(Components.Literal(Text))
			Link.ToolTip = EncodingAttribute(ToolTip)
    ...
    End Function
    Public Function Message(Setting As SubSite, ByVal IDText As Integer, Optional ByVal IDToolTip As Integer = 0, Optional ByVal Href As String = Nothing, Optional ByVal Type As MessageType = MessageType.Normal) As Control
    ...
    Class CacheWidthHeight
      ...
      Property TableImageSize As WHN()
        Get
          SyncLock Cache
            If CBool(Cache.Count) AndAlso Cache.Count <> ElementAtLoad Then
      ...
      Sub LoadWidthHeight(NameFileImage As String, ByRef Width As Integer, ByRef Height As Integer)
        SyncLock Cache
          Try
    ...
    Function Icon(ByVal Logo As IconType, ByVal Skin As Skin, Optional ByVal AlternateText As String = Nothing) As Control
      ...
      Image.Attributes.Add("width", CStr(Width))
      Image.Attributes.Add("height", CStr(Height))
      If Not String.IsNullOrEmpty(AlternateText) Then
    ...
    Function Weather(ByVal WeatherCode As String, ByVal Locality As String, ByVal Language As LanguageManager.Language) As Control
      ...
      If Info.Exist Then
        ...
        Table = Components.Table(5, 2, HorizontalAlign.NotSet, True)
        ...
        Dim LogoEarth As Control = IconUnicode(IconName.Flag, False, Locality & ": " & Phrase(Language, 1034), HrefEarth)
    ...
    Public Function Emoticon(ByVal Name As Common.Emoticons) As WebControls.Image
      ...
      Return Image
    End Function
    Function Button(ByVal Setting As WebApplication.Config.SubSite, ByVal Text As String, ByVal Href As String, Optional ByVal ToolTip As String = Nothing, Optional ByVal Icon As IconType = IconType.Globe, Optional ByVal Target As String = Nothing, Optional ByVal Nofollow As Boolean = False, Optional Itemprop As String = Nothing, Optional Rel As String = Nothing) As Control
      Text = FirstUpper(Text)
      If String.IsNullOrEmpty(ToolTip) Then ToolTip = Text
      ...
      Dim Skin = Setting.Skin()
      Dim SkinSetup = Skin.SkinSetup()
      If SkinSetup IsNot Nothing AndAlso Not StrComp(SkinSetup.Variables("ButtonsWithIcons"), "false", CompareMethod.Text) = 0 Then
        Link.Controls.Add(Components.Icon(Icon, Skin, Text))
    ...
    Sub AddFotoAlbumSlideShow(Setting As SubSite, Button As Control, PhotoAlbum As String)
      If Not String.IsNullOrEmpty(PhotoAlbum) AndAlso Setting.Aspect.AddImageIntoTheBackgroundOfButtonPhotoAlbum Then
        Dim N As Integer = 0
        ...
        If AllPhotos IsNot Nothing AndAlso AllPhotos.Length > 0 Then
          ...
          Dim ScriptControl As WebControl = CType(Components.Script(ScriptCode, ScriptLanguage.javascript), WebControl)
    ...
    Function Script(ByVal ScriptLanguage As ScriptLanguage, ByVal Src As String) As WebControl
      Dim ScriptControl As WebControl = New WebControl(HtmlTextWriterTag.Script)
      ScriptControl.Attributes.Add("src", Src)
      ScriptControl.Attributes.Add("type", ScriptType(ScriptLanguage))
    ...
    Function Script(ByVal Code As String, Optional ByVal ScriptLanguage As ScriptLanguage = ScriptLanguage.javascript, Optional ByVal ForControl As String = Nothing, Optional ByVal ForEvent As String = Nothing) As Control
      Dim ScriptControl As New WebControls.WebControl(HtmlTextWriterTag.Script)
      ScriptControl.EnableViewState = False
      'Ctrl.Attributes("language") = ScriptLanguage.ToString.ToLower
      ScriptControl.Attributes("type") = ScriptType(ScriptLanguage)
      If Not String.IsNullOrEmpty(ForControl) Then
        Select Case ScriptLanguage
          Case Components.ScriptLanguage.vbscript
            ScriptControl.Attributes("for") = ForControl
            ScriptControl.Attributes("event") = ForEvent
          Case Else
            Code = ForControl & "." & ForEvent & "=function " & LetterAndDigit(ForControl) & "_" & ForEvent & "(){" & Code & "}"
        End Select
      End If
      ScriptControl.Controls.Add(New LiteralControl("//<![CDATA[" & vbCrLf & Code & vbCrLf & "//]]>"))
      Return ScriptControl
    End Function
    ...
    Function ScriptOutside(ByVal Source As SourceScript, Optional ByVal ScriptLanguage As ScriptLanguage = ScriptLanguage.javascript) As WebControl
      Dim Script As WebControl = Components.Control(HtmlTextWriterTag.Script)
      Script.Attributes.Add("src", "script.aspx?s=" & Source)
      Script.Attributes.Add("language", ScriptLanguage.ToString)
      Script.Attributes("type") = ScriptType(ScriptLanguage)
      Return Script
    End Function
      ...
      If News IsNot Nothing Then
        If CBool(News.Count) Then
          ...
          If Not Preview Is Nothing Then
            If CBool(MaxElement) Then
              Preview.Controls.Add(Components.Button(Setting, Phrase(Setting.Language, 3090), Href(Setting.Name, False, "default.aspx", QueryKey.Show, DefaultPageShowType.News), Nothing, IconType.Globe))
            End If
    ...
    Sub AddPageArchived(Where As Control, MasterPage As MasterPageEnhanced, Archive As Integer, Page As Integer, HttpContext As HttpContext, DomainConfiguration As DomainConfiguration, Setting As Config.SubSite, Optional ReturnMetaTags As MetaTags = Nothing, Optional HomePage As Boolean = False, Optional AddDefaultLinks As Boolean = True, Optional AddCommentsButton As Boolean = True, Optional Level As Components.LevelMenuItem = LevelMenuItem.Sphera, Optional Category As Integer = 0, Optional CharactersLimit As Integer = 0, Optional AddPdfButton As Boolean = True)
      ...
      Dim MetaTags As Common.MetaTags = New Common.MetaTags(Html)
      Dim CurrentUser As User = Authentication.CurrentUser(MasterPage.Session)

      Dim ShowDate As EnabledStatus = EnabledStatus.Default
      Dim TagShowDate As String = MetaTags.MetaTag("ShowDate")
      Dim AuthorName As String = MetaTags.Author
      Dim Author As User = Authentication.User.Load(AuthorName)
      If Author IsNot Nothing Then
        MasterPage.AddAuthors(Author)
      End If
      If Not String.IsNullOrEmpty(TagShowDate) Then
        ShowDate = CType(Integer.Parse(TagShowDate), EnabledStatus)
      End If
      ...
      Dim TagDl As Boolean = False
      If ShowDate = EnabledStatus.No Then
        If Not HomePage AndAlso CharactersLimit <> 0 AndAlso Setting.SEO.AutoTagDlForThePages Then
          'OLD VERSION NOT C# COMPATIBLE: If Not String.IsNullOrEmpty(MetaTags.Title) AndAlso MetaTags.Title.Count(Function(c As Char) c = " "c) <= 1 Then
          If Not String.IsNullOrEmpty(MetaTags.Title) AndAlso CountChar(MetaTags.Title, " "c) <= 1 Then
            Dim Status As Config.SubSite.CerrelatedStructure.CorrelatedStatus = CType(Convert.ToInt32(MetaTags.Collection("CorrelatedStatus")), Config.SubSite.CerrelatedStructure.CorrelatedStatus)
            If Status = Config.SubSite.CerrelatedStructure.CorrelatedStatus.Enabled Then
              TagDl = True
            End If
          End If
        End If
      ...
      If TagDl Then
        If Not String.IsNullOrEmpty(MetaTags.Title) Then
          Dim Attrib As String = Nothing
          If Not String.IsNullOrEmpty(MetaTags.Description) Then
      ...
      Else
        ...
        If InStr(ReturnMetaTags.KeyWords, MetaTags.Title, CompareMethod.Text) = 0 Then
          ReturnMetaTags.KeyWords &= IfStr(Not String.IsNullOrEmpty(ReturnMetaTags.KeyWords), ",", "") & MetaTags.Title
        End If
        If InStr(ReturnMetaTags.KeyWords, MetaTags.Description, CompareMethod.Text) = 0 Then
          ReturnMetaTags.KeyWords &= IfStr(Not String.IsNullOrEmpty(ReturnMetaTags.KeyWords), ",", "") & MetaTags.Description
        End If
      End If
      ...
      If CharactersLimit = 0 Then
        ...
        If Not String.IsNullOrEmpty(TagPhoto) OrElse Not String.IsNullOrEmpty(TagVideo) Then
          ...
          Else
            HttpContext.Items("CountPhotos") = CInt(HttpContext.Items("CountPhotos")) + 1
          End If

          If CBool(CInt(HttpContext.Items("CountPhotos")) Mod 2) Then
            Align = "PhotoUnpair"
          Else
            Align = "PhotoPair"
          End If
          ...
          If Not String.IsNullOrEmpty(TagPhoto) Then
            Dim PhotoObject As PhotoManager.Photo = PhotoManager.Photo.Load(TagPhoto)
            If PhotoObject IsNot Nothing Then
              Dim Box As HtmlControl = CType(PhotoObject.ControlThumbnail(Setting, False, Config.SizeImagePreview(HttpContext), ThumbnailBoxMode.Thumbnail, DomainConfiguration, HttpContext), HtmlControl)  ' New WebControl(HtmlTextWriterTag.Span)
          ...
          'Add Video
          If Not String.IsNullOrEmpty(TagVideo) Then
            Dim Box As WebControl = CType(VideoObject(TagVideo, Setting, HttpContext), WebControl)
      ...
      'Add poll
      If CharactersLimit = 0 Then
        PollManager.AddPoll(DomainConfiguration, Setting, Body, PollContext.Archine, PollStringToVotes(MetaTags.MetaTag("poll")), Archive, Page, CInt(Setting.Language))
      End If
      ...
      'Add body
      Body = UrlToLink(Body, Setting, DomainConfiguration)
      ...
      If Setting.Aspect.ShowTheAuthorInTheArticles Then
        If Author IsNot Nothing AndAlso Not String.IsNullOrEmpty(Author.Username) Then
          TextContainer.Controls.Add(BR)
      ...
      If AddPdfButton Then
        ...
        If PdfEnabled AndAlso PdfSupported(Setting.Language) Then
          ...
          Dim BtnPdf As WebControl = CType(Components.Button(Setting, "PDF", HrefPdf, Nothing, IconType.Document), WebControl)
      ...
      If CurrentUser.Role(Setting.Name) >= Authentication.User.RoleType.AdministratorJunior Then
        ...
        Dim BtnEdit As WebControl = CType(Components.Button(Setting, Phrase(Setting.Language, 3012), HrefEdit, Nothing, IconType.Pen, Nothing, True), WebControl)
      ...
      If AddCommentsButton = True AndAlso MetaTags.MetaTag("EnabledComments") <> "False" Then
        ...
        'Add comments button
        Dim IdComments As Integer = 0
        If Not String.IsNullOrEmpty(MetaTags.MetaTag("IdComments")) Then
          IdComments = CInt(MetaTags.MetaTag("IdComments"))
        End If

        If Not Setting.DisablingCommentsToPages Then
          'Write a comments
          ...
          Dim BtnWriteComment As WebControl = CType(Components.Button(Setting, Phrase(Setting.Language, 123), HrefWriteComment, Nothing, IconType.Pen, "_blank", True), WebControl)
          MasterPage.Suggest(BtnWriteComment)
          BottomContent.Controls.Add(BtnWriteComment)

          If CBool(IdComments) Then
            'Read a comments
            ...
            Dim BtnComments As WebControl = CType(Components.Button(Setting, Phrase(Setting.Language, 129), HrefComments, Nothing, IconType.Document, Nothing, False, "discussionUrl"), WebControl)
      ...
      If HttpContext.Request.Browser.VBScript = True Then
        If Setting.IsTrue("MicrosoftAgent") Then
          Dim IdButton As Integer = ValInt(CStr(HttpContext.Items("LastIdContent"))) + 1
          HttpContext.Items("LastIdContent") = IdButton
          TextContainer.ID = IdButton.ToString
          BottomContent.Controls.Add(Components.Button(Setting, Phrase(Setting.Language, 3246), "vbscript:SpeechText(" & TextContainer.ClientID & ".innertext)", Nothing, IconType.MusicPlayer))
        End If
      End If

      If CBool(CharactersLimit) Then
        'Show all
        Dim BtnComments As WebControl = CType(Components.Button(Setting, Phrase(Setting.Language, 117), Canonical, Nothing, IconType.Document, Nothing, False, "url"), WebControl)
        BottomContent.Controls.Add(BtnComments)
      End If
      ...
      If CharactersLimit = 0 Then
        ...
        If Not String.IsNullOrEmpty(NamePhotoAlbum) Then
    ...
    Function IFrame(ByVal Width As String, ByVal Height As String, ByVal Src As String, Optional ByVal HtmlNoFrameSupport As String = Nothing, Optional Sandbox As String = "") As Control
      ...
      If Not String.IsNullOrEmpty(HtmlNoFrameSupport) Then
    ...
    Function PhoneNumber(ByVal PhoneContact As Config.Configuration.ContactsConfiguration.PhoneContact, ByVal Language As LanguageManager.Language, ByVal Skin As Skin) As Control
      ...
      Dim Icon As WebControls.Label = CType(IconUnicode(IconName.Thelephone, True), Label)
    ...
    Function IconUnicode(Name As Integer, Optional Big As Boolean = False, Optional ToolTip As String = Nothing, Optional Href As String = Nothing, Optional Target As String = Nothing, Optional NoFollow As Boolean = True) As WebControl
    ...
    Function LabelWithIcon(IconUnicodeValue As Integer, Optional Text As String = Nothing, Optional Href As String = Nothing, Optional Target As String = Nothing, Optional NoFollow As Boolean = True) As Control
      Dim Label As New WebControls.HyperLink
      Label.CssClass = "label"
      Label.Controls.Add(IconUnicode(IconUnicodeValue))
    ...
    Function CallCenter(ByVal Setting As SubSite) As Control
      Dim ShowFreeNumber As Boolean = False
      If Cookie("IsCustomer") <> "1" Then
        ShowFreeNumber = True
      End If
      Dim PhoneContact As Config.Configuration.ContactsConfiguration.PhoneContact = Setting.PhoneContact(Configuration.ContactsConfiguration.Service.CustomerCare, ShowFreeNumber)
    ...
    Public MustInherit Class MasterPageEnhanced
      ...
      Public MustOverride Sub AddItemService(ByVal Href As String, ByVal Text As String, ByVal ToolTip As String, Optional ByVal Level As Components.LevelMenuItem = LevelMenuItem.Topic)
      Public MustOverride Property User As Authentication.User
      Public MustOverride Property DomainConfiguration As Config.DomainConfiguration
      Public MustOverride Property Setting As Config.SubSite
      ...
      Public MustOverride Sub AddButton(ByVal Button As ButtonsStandard, Optional Suggest As Boolean = False)
      Public MustOverride Sub ShowElement(Optional LeftBar As Boolean = True, Optional RightBar As Boolean = True, Optional TopBar As Boolean = True, Optional BottomBar As Boolean = True)
      ...
      Public MustOverride Sub Suggest(Control As Control)
      ...
      Public MustOverride Sub AddMessage(Setting As SubSite, ByVal IDText As Integer, Optional ByVal IDToolTip As Integer = 0, Optional ByVal Href As String = Nothing, Optional ByVal Type As MessageType = MessageType.Normal)
    ...
    Function OnlineUser(Setting As SubSite, Optional OnlyThisDomain As Boolean = False, Optional Link As Boolean = True) As Control
      Dim ListRegistered As New WebControl(HtmlTextWriterTag.Div), ListNotRegistered As New WebControl(HtmlTextWriterTag.Div), ListCrawler As New WebControl(HtmlTextWriterTag.Div)
      ...
      Dim CountRegistered As Integer = 0, CountNotRegistered As Integer = 0, CountCrawler As Integer = 0
      SyncLock OnLineUsers
        ...
        For Each Online As OnlineUser In OnLineUsers.Values
          User = Online.User
          If Not OnlyThisDomain OrElse MyDomain = Online.Domain OrElse Not String.IsNullOrEmpty(User.Username) Then
    ...
    Public Sub AddCells(Row As HtmlTableRow, Cells As Integer, Optional Nowrap As Boolean = False)
    ...
    Public Sub AddHeaderCells(Row As HtmlTableRow, Cells As Integer, Optional Nowrap As Boolean = False)
      For n As Integer = 1 To Cells
    ...
    Public Function Row(Cells As Integer, Optional HAlign As HorizontalAlign = HorizontalAlign.NotSet, Optional Nowrap As Boolean = False) As HtmlTableRow
    ...
    Public Function HeaderRow(Cells As Integer, Optional HAlign As HorizontalAlign = HorizontalAlign.NotSet, Optional Nowrap As Boolean = False) As HtmlTableRow
      Dim NRow As New HtmlTableRow
      If HAlign <> HorizontalAlign.NotSet Then
        NRow.Style.Add("text-align", LCase(HAlign.ToString)) 'html5
      End If
    ...
    Sub AddLabel(Control As HtmlTableCell, Text As String, Optional VerticalText As Boolean = False)
    ...
    Function Link(NavigateUrl As String, Optional Text As String = Nothing, Optional ToolTip As String = Nothing, Optional Bold As Boolean = False, Optional Target As String = Nothing, Optional Rel As Rel = Rel.None, Optional Itemprop As String = Nothing) As WebControls.HyperLink
      Dim ReturnLink As WebControls.HyperLink = New WebControls.HyperLink
      If Itemprop IsNot Nothing Then
        ReturnLink.Attributes.Add("itemprop", Itemprop)
      End If
      If Rel <> Components.Rel.None Then
        ReturnLink.Attributes("rel") = Rel.ToString.ToLower()
      End If
      ReturnLink.NavigateUrl = NavigateUrl
      If Bold Then
        ReturnLink.Style.Add("font-weight", "bold !important")
      End If
      ReturnLink.Text = HttpUtility.HtmlEncode(Text)
      ReturnLink.ToolTip = EncodingAttribute(ToolTip)
    ...
    Sub AddLiteral(Control As Control, Text As String)
      Dim Literal As New WebControls.Literal
      Literal.Text = Text
      Control.Controls.Add(Literal)
    End Sub

    Function Table(Rows As Integer, Columns As Integer, Optional HAlign As HorizontalAlign = HorizontalAlign.NotSet, Optional Nowrap As Boolean = False, Optional FirstIsHeader As Boolean = False) As HtmlTable
    ...
    Function FlashControl(FlashFile As String, Width As Integer, Height As Integer, Optional Hide As Boolean = False) As Control
    ...
    Public Function ModPLayer() As WebControls.WebControl
      ...
      Control = CType(FlashControl(FlashResources & "/BitboyApp.swf", 150, 57, Setup.RenderingEngine.HideModPlayer), WebControl)
    ...
    Function Folder(Text As String, ToolTip As String, Href As String, Setting As Config.SubSite, Optional Level As Integer = 0, Optional FolderType As FolderType = Components.FolderType.Normal) As Control
      'Dim Text
      Dim Icon As IconType = IconType.Folder
    ...
    Sub AddTagSamp(Html As String)
      Dim Html2 As String = HtmlMapping(Html)
      Dim Paragraphs As String() = Split(Html2, vbLf)
      Dim Flag As Boolean = False
      Dim Position As Integer = 0, EndLinePosition As Integer = 0, CountLine As Integer = 0
      ...
      For Each Paragraph As String In Paragraphs
        ...
        If Position <> 0 Then
          ...
          Else
            If Not String.IsNullOrEmpty(Trim(Paragraph)) Then
              If Flag Then
                Dim StartEnd As New Position(Position, EndLinePosition)
                CodeBlock.Add(StartEnd)
                Flag = False
            ...
            End If
          End If
          If CountLine = Paragraph.Length AndAlso Flag = True Then
            Dim StartEnd As New Position(Position, EndLinePosition)
            CodeBlock.Add(StartEnd)
          End If
        End If
    ...
    Private Function IsCode(ByVal Html As String) As Boolean
      ...
      'Remove emoticons
      If Not String.IsNullOrEmpty(TrimText) AndAlso TrimText.IndexOfAny("()-|".ToCharArray) <> -1 Then
      ...
      If Not String.IsNullOrEmpty(TrimText) Then
        Dim LastChr As Char = TrimText(TrimText.Length - 1)
        If (Not Char.IsPunctuation(LastChr) OrElse LastChr = ")"c OrElse LastChr = "}"c OrElse LastChr = "{"c) OrElse LastChr = ";"c <> false Then
          ...
          For Each Chr As Char In TrimText
            ...
            If Char.IsNumber(Chr) Then
              Finded = True
            ElseIf Chr = ")"c AndAlso Finded Then
              Return False
            Else
              Exit For
            End If
          ...
          If TrimText.StartsWith("http://") OrElse TrimText.StartsWith("https://") OrElse TrimText.StartsWith("www.") Then
            Return False
          End If

          'Veryfi if is a Rem
          If TrimText.Contains("Rem ") Then
            Return True
          End If
          If TrimText(0) = "'"c AndAlso LastChr <> "'"c Then
            Return True
          End If
          If TrimText.Contains("//") AndAlso Not TrimText.Contains("://") Then
            'JavaScript Rem
            Return True
          End If
          If TrimText.First = "*"c OrElse TrimText.StartsWith("*/") Then
            'CSS Rem
            Return True
          End If

          'Verify if is a CSS class definition
          If TrimText.First = "."c Then
            Return True
          End If

          'Verify if is a html code
          If TrimText.Contains("<"c) AndAlso TrimText.Contains(">"c) Then 'Contains in c# don't accept char value
            Return True
          End If
          If TrimText.First = "<"c OrElse TrimText.Last = ">"c Then 'EndsWith in c# don't accept char value
            Return True
          End If

          'Verify if is a html JavaScript
          If TrimText.Contains("{"c) OrElse TrimText.Contains("}"c) Then 'Contains in c# don't accept char value
            Return True
          ElseIf TrimText.Last = ";"c Then
            'no c# compatibe: ElseIf TrimText.Last = ";"c Then
            Return True
          End If
          ...
          If InStr(TrimText, "=") <> 0 OrElse InStr(TrimText, ">") <> 0 OrElse InStr(TrimText, "<") <> 0 Then
            ...
            Dim MathKeys As Char() = {"+"c, "-"c, "/"c, "\"c, "^"c, "*"c}
            For Each MathKey As Char In MathKeys
              If TrimText.Contains(MathKey) Then
                Return True
              End If
            Next
          ...
          Dim PartOfText As Boolean = False, FirstWordFinded As Boolean = False, HaveUpper As Boolean = False
          For Each Chr As Char In TrimText
          ...
          Dim LastChar As Char = TrimText(TrimText.Length - 1)  'TrimText.Last no compatible c#
          ...
          Dim IsNotSimpleText As Boolean = False
          For Each Chr As Char In TrimText
    ...
    Function DateControl([Date] As Date, Setting As SubSite) As HtmlControl
      ...
      PubDateLabel.InnerText = [Date].AddSeconds(CurrentUser(Nothing).TimeOffsetSeconds).ToString(Setting.DateFormat, Setting.Culture())
      Return PubDateLabel
    End Function
    ...
    Function QuickInfoUser(ByVal Setting As SubSite, ByVal User As User, Optional ByVal Forum As Forum = Nothing, Optional ByVal IsPreview As Boolean = False, Optional ByVal Domain As DomainConfiguration = Nothing) As Control
      ...
      Else
        ...
        If Extension.IsCrawler() Then
          ...
          Dim GenderType As User.GenderType = User.Gender
          Gender.Controls.Add(TextControl(GenderType.ToString().ToLower()))
            ...
            Select Case 1
              ...
              Case 2
                ...
                If Not User.PhotoAlbum Is Nothing Then
                  HyperLink.NavigateUrl = HrefPhotoAlbum(User.NamePhotoAlbum, Setting, Domain)
                End If
            End Select
          End If
        ...
        ElseIf User.GeneralRole < Authentication.User.RoleType.Visitors Then
          'Banned
          Ctrl.Controls.Add(IconUnicode(IconName.Block, False, Phrase(Setting.Language, 3193)))
        End If
        ...
        If BlockToDate <> Nothing Then
          Ctrl.Controls.Add(IconUnicode(IconName.Block, False, Phrase(Setting.Language, 3194) & " " & BlockToDate.AddSeconds(CurrentUser.TimeOffsetSeconds).ToString(Setting.DateTimeFormat(), Setting.Culture())))
        End If

        If User.GeneralRole < Authentication.User.RoleType.Senior Then
          'Cloned
          If Not String.IsNullOrEmpty(User.Generate) Then
        ...
        If User.IsOnline Then
          Ctrl.Controls.Add(IconUnicode(IconName.Sun, False, Phrase(Setting.Language, 3040)))
          ...
          If CBool(PrivateConversation(User, CurrentUser)) Then
            ...
            Ctrl.Controls.Add(IconUnicode(IconName.Conference, False, Phrase(Setting.Language, 3230), Href, ChatTarget(CurrentUser.Username)))
          End If
        End If

        'Skype
        If CurrentUser.GeneralRole >= Authentication.User.RoleType.Senior AndAlso Not String.IsNullOrEmpty(User.Skype) Then
        ...
        If Forum IsNot Nothing Then
          If (CBool(Stars(CurrentUser)) AndAlso Forum.Type <> ForumManager.Forum.TypeOfForum.TicketSystem) OrElse AbleToAdmonish(Setting) Then
            If User.Role(Setting.Name) <= Authentication.User.RoleType.Senior Then
              Dim NavigateUrl As String = Href(Setting.Name, False, "messenger.aspx", MessengerManager.QueryStringParameters(User.Username, MessengerManager.Action.RequireAdmonishUser, False, Nothing, Forum.Id))
              Dim Admonish As Control = IconUnicode(IconName.ScissorBlack, False, Phrase(Setting.Language, 3191), NavigateUrl)
