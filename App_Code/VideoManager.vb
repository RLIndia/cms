...
Namespace WebApplication
	Public Module VideoManager
      ...
      'If Url.StartsWith("http://www.facebook.com/") OrElse Url.StartsWith("https://www.facebook.com/") Then
      '  Return Url
      'End If

			Try
				Dim Mime As String = MimeType(Url)
        If Not String.IsNullOrEmpty(Mime) AndAlso Mime.StartsWith("video") Then
          Return Url & "|" & Mime
        End If
      ...
      If Url.StartsWith("http://youtu.be/") AndAlso Url <> "http://youtu.be/" Then
        Url = Navigator.ResolveUrlRedirect(Url)
      End If
    ...
    Public Function IsMediaSource(ByVal Url As String) As Boolean
      Select Case GetFileExtension(Url)
    ...
    Function MimeType(ByVal Url As String) As String
      ...
      WebRequest = CType(System.Net.HttpWebRequest.Create(Url), Net.HttpWebRequest)
    ...
    Function VideoObject(ByVal VideoId As String, Setting As SubSite, ByVal HttpContext As HttpContext) As Control
      ...
      'Width = 600
      Height = CInt(Width / 16 * 10)
      If VideoId IsNot Nothing AndAlso VideoId.Contains("//") Then
        If VideoId.StartsWith("http://www.ustream.tv/recorded/") Then
          'ustream.tv
          VideoId = Extension.Right(VideoId, 32)
          Height = CInt(Width / 16 * 9) '16/9 resolution
          ...
          'ElseIf VideoId.StartsWith("http://www.facebook.com/") OrElse VideoId.StartsWith("https://www.facebook.com/") Then
          '  'Facebook.com
          '  Dim Uri As New Uri(VideoId)
          '  Dim NameValue As NameValueCollection = HttpUtility.ParseQueryString(Uri.Query)
          '  VideoId = NameValue.Get("v")
          '  If VideoId <> "" Then
          '    Height = CInt(Width / 16 * 9) '16/9 resolution
          '    'This code need a cerrection!!
          '    Literal.Text = "<iframe src='https://www.facebook.com/video/embed?video_id=" & VideoId & "' " & VideoSize(Setting, Width, Height) & " frameborder='0'></iframe>"
          '  End If
        Else
          ...
          If UseVideoPlayer = VideoPlayer.Silverlight AndAlso HttpContext.Current.Request.Browser.Browser = "IE" AndAlso ValInt(CStr(HttpContext.Current.Items("SilverlightVideo"))) > 0 Then
          ...
          ElseIf UseVideoPlayer = VideoPlayer.Silverlight Then
            ...
            HttpContext.Current.Items("SilverlightVideo") = ValInt(CStr(HttpContext.Current.Items("SilverlightVideo"))) + 1
      ...
      Else
        Select Case Len(VideoId)
          ...
          Case 11
            ...
            'No plugin (html5)
            Literal.Text = "<iframe " & VideoSize(Setting, Width, Height) & " src=""http://www.youtube.com/embed/" & VideoId & """></iframe>"
            ...
            If Extension.IsCrawler() Then
              ...
              End If
            End If
            Literal.Text = "<span itemscope=""itemscope"" itemtype=""http://schema.org/VideoObject""><meta itemprop=""thumbnailUrl"" content =""http://i3.ytimg.com/vi/" & VideoId & "/hqdefault.jpg"" />" & AddSourge & Literal.Text & "</span>"
          Case 11, 12, 13, 14, 15, 16, 17
            'Facebook video
            If Long.TryParse(VideoId, Nothing) Then 'verify is VideoId is a number
              Height = CInt(Width / 16 * 9) '16/9 resolution
              'This code need a cerrection!!
              Literal.Text = "<iframe src='https://www.facebook.com/video/embed?video_id=" & VideoId & "' " & VideoSize(Setting, Width, Height) & " frameborder='0'></iframe>"
            End If
