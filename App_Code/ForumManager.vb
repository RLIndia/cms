...
Namespace WebApplication
      ...
      End Function
        ...
        For Each Category As Category In Categories
          For Each SubCategory As ForumStructure.Category.Subcategory In Category.Subcategories
            If SubCategory.Id > ReturnValue Then
              ReturnValue = SubCategory.Id
            End If
          Next
        Next
        Return ReturnValue
    ...
    End Class
      ...
      Select Case ForumId
        Case ReservedForums.ArchiveComment, ReservedForums.PhotoComment, ReservedForums.PrivateMessage
          Return True
        Case Else
          Return False
      End Select
      ...
      Dim Root As String = FileManager.MapPath(ForumSubDirectory)
      ...
      For Each Folder As System.IO.FileSystemInfo In Folders
        Try
          ...
          If WithoutForSystemUse = False OrElse ForumForInsideUse(ForumId) = False Then
    ...
    Class Forum
      ...
      Public Function TitleControl(Setting As SubSite, Optional SubCategory As ForumStructure.Category.Subcategory = Nothing, Optional IsTopicsNotClassified As Boolean = False, Optional Title As String = Nothing, Optional Description As String = Nothing) As Control
        Dim Control As New Control
        If Admitted(Id, Setting) Then
          Dim Href As String
          Dim TotalTopics As Integer = 0
          ...
          If IsTopicsNotClassified Then
            ...
            Href = ForumManager.Link(ForumManager.ActionType.Show, Setting.Name, False, Nothing, Id, 0, 0, 0, Nothing, Nothing, False, Nothing, 0)
          ...
          Else
            Href = ForumManager.Link(ForumManager.ActionType.Show, Setting.Name, False, Nothing, Id, 0, 0, 0, Nothing, Nothing, False, Nothing, SubCategory.Id)
          ...
          Dim User As User = CurrentUser(Nothing)
      ...
      Function Summary(ByVal Setting As SubSite) As Control
        Dim Control As HtmlGenericControl = New HtmlGenericControl("section")
        If Admitted(Id, Setting) Then
          Control.Controls.Add(Me.TitleControl(Setting))
          If CBool(ForumStructure.Categories.Count) Then
            Dim Table As New HtmlTable
            Control.Controls.Add(Table)
            Dim Categories As System.Collections.Generic.List(Of ForumStructure.Category) = Me.ForumStructure.CategoriesSorted
            For Each Category As ForumStructure.Category In Categories
              ...
              For Each SubCategory As ForumStructure.Category.Subcategory In Category.Subcategories
                ...
                Row.Cells.Add(New HtmlTableCell)
                Row.Cells.Add(New HtmlTableCell)
                Row.Cells(0).Controls.Add(EnhancedLinkIcon(Setting.Skin(), SubCategory.Icon, Nothing, Me.Href(Setting, SubCategory)))
              ...
              SyncLock ForumArchiveDetected
                If Not ForumArchiveDetected.Contains(Id) Then
                  ...
                  For Each Setting As SubSite In AllSubSite()
                    ...
                    If Archive <> 0 AndAlso Setting.Forums IsNot Nothing AndAlso Setting.Forums.Length = 1 AndAlso Setting.Forums(0) = Id Then
      ...
      Private Function HrefEdit(ByVal Setting As SubSite) As String
        Return ForumManager.Link(ForumManager.ActionType.Modify, Setting.Name, False, Nothing, Id)
      End Function

      Function Tools(ByVal Setting As SubSite) As Control
        Dim Edit As Control = Components.IconUnicode(IconName.Pencil, False, Phrase(Setting.Language, 74, 61), HrefEdit(Setting))
        Return Edit
      End Function

      Sub AddButtons(ByVal Setting As SubSite, ByVal ToMasterPage As MasterPageEnhanced)
        ToMasterPage.AddButton(Phrase(Setting.Language, 74, 61), HrefEdit(Setting), Nothing, Components.IconType.ControlPanel, Components.MasterPageEnhanced.TargetForButton.Blank, True)
      End Sub

      ReadOnly Property PhotoAlbum As String
        Get
          Return "forum/" & Id
        End Get
      End Property
      ...
      Shared Function NewForum(ByVal Name As String, ByVal Description As String) As Forum
        Dim ID As Integer = NewSubDirectoryId(FileManager.MapPath(ForumSubDirectory), 200)
      ...
      Function Href(ByVal SubSite As SubSite, Optional SubCategory As ForumStructure.Category.Subcategory = Nothing) As String
        If SubCategory IsNot Nothing Then
          Return Link(ActionType.Show, SubSite.Name, False, Nothing, Me.Id, 0, 0, 0, Nothing, Nothing, False, Nothing, SubCategory.Id)
        Else
          Return Link(ActionType.Show, SubSite.Name, False, Nothing, Me.Id)
        End If
      End Function
      ...
      Function Trace(ByVal Setting As SubSite, Optional SubCategory As Integer = -1) As Control()
        If Me.ForumStructure IsNot Nothing Then
          ...
          NewTrace.AddElement(Me.Name, Me.Description, Link(ActionType.Show, Setting.Name, False, Nothing, Me.Id))
          If SubCategory = 0 Then
            NewTrace.AddElement(Phrase(Setting.Language, 137), Phrase(Setting.Language, 138), Link(ActionType.Show, Setting.Name, False, Nothing, Me.Id, 0, 0, 0, Nothing, Nothing, False, Nothing, SubCategory) & "#SubCategory" & SubCategory)
          ...
          Else
            If SubCategory > 0 AndAlso Category IsNot Nothing Then
              ...
              NewTrace.AddElement(Category.Title, Category.Title, Link(ActionType.Show, Setting.Name, False, Nothing, Me.Id) & "#Category" & IdCategory)
              NewTrace.AddElement(SubCategoryObject.Title, SubCategoryObject.Description, Link(ActionType.Show, Setting.Name, False, Nothing, Me.Id, 0, 0, 0, Nothing, Nothing, False, Nothing, SubCategory))
            End If
            Return NewTrace.Controls()
          End If
        End If
      ...
      Function GetAllTopicsId() As Integer()
        ...
        If IO.Directory.Exists(DirectoryName) Then
          ...
          Dim Result(Directories.Length - 1) As Integer
          Dim Index As Integer = 0
          For Each Directory As IO.DirectoryInfo In Directories
            Result(Index) = ValInt(Directory.Name)
            Index += 1
          Next
          Return Result
        Else
    ...
    Private Sub LoadCache(ByVal ForumId As Integer)
      Dim Root As String = FileManager.MapPath(ForumSubDirectory & "\" & ForumId)
      Dim Dir As New System.IO.DirectoryInfo(Root)
      ...
      For Each EachTopic As Topic In AllTopics.Values
        If Not Array.IndexOf(Ids, EachTopic.ID) <> -1 Then
          Topic.Delete(ForumId, EachTopic.ID)
        End If
      Next

      'Add to cache not loaded topics
      Dim CacheIsChanged As Boolean = False
      For Each TopicID As Integer In Ids
        If Not AllTopics.ContainsKey(TopicID) Then
          ForumManager.Topic.Load(ForumId, TopicID)
          CacheIsChanged = True
        End If
      Next
    ...
    Public Function FindLastReply(ByVal Topic As Topic, Optional ByVal FindLastNotCensored As Boolean = False, Optional ByVal FindNoHide As Boolean = False) As Reply
      ...
      If CBool(Files.Length) Then
        If FindLastNotCensored OrElse FindNoHide Then
          For N As Integer = Files.GetUpperBound(0) To Files.GetLowerBound(0) Step -1
            Dim ReplyID As Integer = ValInt(Files(N).Name)
          ...
          Next
        Else
          Dim IdLastReply As Integer = ValInt(Files(Files.GetUpperBound(0)).Name)
          Return ForumManager.Reply.Load(Topic.ForumId, Topic.ID, IdLastReply)
        End If
      End If
      Return Nothing
    End Function
    ...
    Public Function LastReplyId(ForumId As Integer, TopicId As Integer) As Integer
      ...
      SyncLock Obj
        ...
        Dim Files() As System.IO.FileInfo = Dir.GetFiles("*.htm")
        If CBool(Files.Length) Then
          Return ValInt(Files(Files.GetUpperBound(0)).Name)
        End If
      End SyncLock
      Return 0
    End Function
    ...
    Private Function PathForum(ByVal ForumId As Integer) As String
      Return FileManager.MapPath(ForumSubDirectory & "\" & ForumId & "\")
    End Function
    ...
    Function ShowTopicsByCategory(ByVal User As User, ByVal Setting As SubSite, ByVal ForumId As Integer, ByVal ShowCensored As Boolean, Optional ByVal Page As Page = Nothing) As Control
      If Admitted(ForumId, Setting) Then
        ...
        If Page IsNot Nothing Then
          ...
          Dim Master As Components.MasterPageEnhanced = CType(Page.Master, Components.MasterPageEnhanced)
          ...
          Master.MetaRevisitAfterDays = 1
          Master.SetTrace(Forum.Trace(Setting))
        End If
        ...
        If Forum.ForumStructure IsNot Nothing Then
          ...
          Dim Groups As System.Collections.Generic.Dictionary(Of Integer, System.Collections.Generic.List(Of ForumManager.Topic)) = New System.Collections.Generic.Dictionary(Of Integer, System.Collections.Generic.List(Of ForumManager.Topic))
          ...
          Groups.Add(0, WithoutSubcategories)
          For Each Category As ForumStructure.Category In Categories
            For Each Subcategory As ForumStructure.Category.Subcategory In Category.Subcategories
              Dim Subcategories As New System.Collections.Generic.List(Of ForumManager.Topic)
              If Not Groups.ContainsKey(Subcategory.Id) Then
                Groups.Add(Subcategory.Id, Subcategories)
              End If
            Next
          Next
          ...
          For Each EachTopic As Topic In Topics
            If TopicMomentarilyHidden(EachTopic) = False OrElse ShowHidden Then
              If EachTopic.Censored = False OrElse ShowCensored = True Then
                Dim Id As Integer
                If EachTopic.SubCategory < 0 OrElse Not Groups.ContainsKey(EachTopic.SubCategory) Then
                  Id = 0
                Else
                  Id = EachTopic.SubCategory
                End If
                Groups(Id).Add(EachTopic)
              End If
            End If
          Next

          'Sort
          For Each Group As System.Collections.Generic.List(Of ForumManager.Topic) In Groups.Values
            SortTopic(ShowCensored, ShowHidden, User.Username, Group)
          Next

          If Not String.IsNullOrEmpty(CacheKey) Then
            'Update TotalTopics by category
            Dim SaveForum As Boolean = False
            For Each Category As ForumStructure.Category In Categories
              For Each Subcategory As ForumStructure.Category.Subcategory In Category.Subcategories
    ...
    Function Admitted(ForumId As Integer, Setting As SubSite) As Boolean
      If Setting.Forums IsNot Nothing Then
        Return Array.IndexOf(Setting.Forums, ForumId) <> -1
      End If
      Return False
    End Function
    ...
    Function ShowTopicsList(ByVal User As User, ByVal Setting As SubSite, ByVal ForumId As Integer, ByVal ShowCensored As Boolean, Optional ByVal CurrentPage As Integer = 0, Optional ByVal TopicsInPreview As Integer = 0, Optional ByVal SubCategory As Integer = -1, Optional ByVal Action As ActionType = ActionType.Show, Optional ByVal FindText As String = Nothing, Optional ByVal Page As Page = Nothing, Optional ByVal SetMetaRevisitAfter As Boolean = False, Optional ByVal Topics As List(Of ForumManager.Topic) = Nothing) As Control
      ...
      If Admitted(ForumId, Setting) Then
        ...
        Dim TopicsNotClassified As Boolean = False
        If SubCategory = 0 Then
        ...
        Dim CacheKey As String = Nothing
        If Common.WebCache IsNot Nothing Then
          If String.IsNullOrEmpty(User.Username) AndAlso Action = ActionType.Show AndAlso CurrentPage = 1 Then
        ...
        For EachC As Integer = c.GetLowerBound(0) To c.GetUpperBound(0)
          ...
          Select Case NC
            Case 0  'Logo
              'Set RSS link
              TitleRow.Controls(EachC).Controls.Add(RssButton(ForumManager.Link(ForumManager.ActionType.ShowRSS, Setting.Name, False, Nothing, ForumId, 0, 0, 0, Nothing, Nothing, False, Nothing, SubCategory)))
        ...
        If CurrentForum IsNot Nothing Then
          ...
          If Topics IsNot Nothing AndAlso Topics.Count <> 0 Then
            ...
            For NTopic As Integer = FirstTopic To LastTopic
              Dim Topic As Topic = Topics(NTopic) 'c# (NTopic)->[NTopic]
              Dim FirstReply As Reply = Topic.FirstReply()
              If FirstReply Is Nothing Then
                Extension.Log("FirstReplyIsNothing", 1000, CStr(ForumId), CStr(NTopic))
              Else
                ...
                Dim FlagRevisitAfter As Boolean = False
                ...
                Dim Row As HtmlTableRow = Components.Row(c.GetUpperBound(0) + 1, HorizontalAlign.NotSet, True)
                ...
                For EachC As Integer = c.GetLowerBound(0) To c.GetUpperBound(0)
                  ...
                  Select Case NC
                    Case 0  'folder
                      ...
                      Row.Cells(EachC).Controls.Add(EnhancedLinkIcon(Setting.Skin(), Folder, Nothing, Reply.QueryString(Setting, ForumId, Topic.ID, Topic.TotalPages(ShowCensored))))
                    ...
                    Case 1  'author & topic
                      ...
                      Dim InfoUser As RenderMethod = Sub(Writer As System.Web.UI.HtmlTextWriter, Container As System.Web.UI.Control)
                                                       If FirstReply.AuthorAccount Is Nothing Then
                                                         Components.QuickInfoUser(Config.CurrentSetting(), Authentication.User.Load(FirstReply.Author), CurrentForum).RenderControl(Writer)
                                                       Else
                                                         Components.QuickInfoUserExternalAccount(FirstReply.Author, FirstReply.AuthorAccount).RenderControl(Writer)
                                                       End If
                                                     End Sub
                    ...
                    Case 2  'replies
                      Row.Cells(EachC).Style.Add("text-align", "center") 'html5
                      Row.Cells(EachC).InnerText = (IfInt(ShowCensored, Topic.NReplies, Topic.NRepliesNotCensored) - 1).ToString()
                    Case 3  'readed
                      Row.Cells(EachC).Style.Add("text-align", "center") 'html5
                    ...
                    Case 4  'Last user & Last message
                      ...
                      If LastReply IsNot Nothing Then 'Prevent error
                        ...
                        Dim InfoUser As RenderMethod = Sub(Writer As System.Web.UI.HtmlTextWriter, Container As System.Web.UI.Control)
                                                         If FirstReply.AuthorAccount Is Nothing Then
                                                           Components.QuickInfoUser(Config.CurrentSetting(), Authentication.User.Load(LastReply.Author), CurrentForum).RenderControl(Writer)
                                                         Else
                                                           Components.QuickInfoUserExternalAccount(LastReply.Author, LastReply.AuthorAccount).RenderControl(Writer)
                                                         End If
                                                       End Sub
                    ...
                    Case 5  'operations
                      ...
                      Dim Bookmark As String = ForumManager.Link(ForumManager.ActionType.Show, Setting.Name, True, Nothing, ForumId, Topic.ID, 0, 1)
                      Dim Bookmarks As Control = Components.IconUnicode(IconName.BlackStar, False, Phrase(Setting.Language, 134), HrefGoogleAddBookmarks(Setting.Language, Topic.Title, Bookmark, PathCurrentUrl), "_blank")

                      Row.Cells(EachC).Controls.Add(Bookmarks)

                      If (CBool(Stars(User)) AndAlso CurrentForum.Type <> Forum.TypeOfForum.TicketSystem) OrElse AbleToCensore(Setting) Then
                        'Censore topic
                        Dim NavigateUrl As String = Href(Setting.Name, False, "messenger.aspx", MessengerManager.QueryStringParameters(, MessengerManager.Action.RequireTopicCensore, False, Nothing, ForumId, Topic.ID, 1))
                        Dim Censore As Control = Components.IconUnicode(IconName.ScissorBlack, False, Phrase(Setting.Language, 3075), NavigateUrl)
                        Row.Cells(EachC).Controls.Add(Censore)
                      End If

                      If AbleToCensore(Setting) Then
                        'Delete topic
                        Dim NavigateUrl As String = Href(Setting.Name, False, "messenger.aspx", MessengerManager.QueryStringParameters(, MessengerManager.Action.DeleteTopics, False, Nothing, ForumId, Topic.ID, 1))
                        Dim Delete As Control = Components.IconUnicode(IconName.ScissorWhite, False, Phrase(Setting.Language, 51), NavigateUrl)
            ...
            If Not IsPreview Then
              If TotalPage > 1 Then
                Ctrl.Controls.Add(Paginate(Setting, TotalPage, CurrentPage, Action, ForumId, 0, FindText, SubCategory))
              End If
            End If
          Else
          ...
          If Not IsPreview Then
            If User.Role(Setting.Name) >= Authentication.User.RoleType.Visitors Then
              ...
              Href = ForumManager.Link(ForumManager.ActionType.NewTopic, Setting.Name, False, Nothing, ForumId, 0, 0, 0, Nothing, Nothing, False, Nothing, SubCategory)
              Dim NewTopicButton As Control = Components.Button(Setting, Text, Href, Nothing, Components.IconType.DocumentBlank, Nothing, True)
              If MasterPage IsNot Nothing Then
                MasterPage.AddButton(Text, Href, Nothing, Components.IconType.DocumentBlank, MasterPageEnhanced.TargetForButton.Self, True, Nothing, True)
                MasterPage.Suggest(NewTopicButton)
              End If
    ...
    Private Function CurrentForum(ByVal Setting As SubSite, ByVal ForumId As Integer) As Forum
      If Setting.Forums IsNot Nothing Then
        If Array.IndexOf(Setting.Forums, ForumId) <> -1 Then
          Return CType(ForumManager.Forum.Load.GetItem(ForumId), Forum)
        End If
      End If
    ...
    Function IconOfTopic(ByVal Topic As Topic, ByVal User As User) As Components.IconType
      ...
      Dim HasWritedInTopic As Boolean = False
      ...
      If FirsReply IsNot Nothing Then
        ...
        ElseIf Not String.IsNullOrEmpty(FirsReply.Video) Then
          If HasWritedInTopic Then
            Return Components.IconType.FolderVideo1
          Else
            Return Components.IconType.FolderVideo
          End If
        ElseIf Not String.IsNullOrEmpty(FirsReply.Photo) Then
          If HasWritedInTopic Then
    ...
    Sub AddRssReference(ByVal Page As Page, ByVal Setting As SubSite, ByVal ForumId As Integer, Optional ByVal SubCategory As Integer = -1)
      ...
      Meta.Href = ForumManager.Link(ForumManager.ActionType.ShowRSS, Setting.Name, False, Nothing, ForumId, 0, 0, 0, Nothing, Nothing, False, Nothing, SubCategory)
      Page.Header.Controls.Add(Meta)
    End Sub

    Function IsBlockedUntil(ByVal UserToCheck As Authentication.User) As Date
      'Check if user is temporary blocked
      Dim Admonition As String = CStr(UserToCheck.Attribute("Admonition"))
      Dim LastAmmonition As Date
      Dim Value As Integer = 0
      If Not String.IsNullOrEmpty(Admonition) Then
        Dim Data() As String = Split(Admonition, " ")
        LastAmmonition = DateTime.FromOADate(Microsoft.VisualBasic.Val(Data(0)))
        Value = CInt(Data(1))
        If Value >= Setup.Security.QuorumToBlockUser Then
          ...
          Else
            'Unblock user
            UserToCheck.Attribute("Admonition") = Nothing
            UserToCheck.Attribute("UserAdmonish") = Nothing
          End If
        End If
      End If
      Return Nothing
    ...
    Public Sub RequireAdmonish(ByVal Setting As SubSite, ByVal UserName As String, ByVal ForumId As Integer)
      ...
      If HttpContext.Current.Session(NameFlag) Is Nothing Then
        ...
        Dim UserToAdmonish As Authentication.User = Authentication.User.Load(UserName)
        If Not UserToAdmonish Is Nothing Then
          Dim UserAdmonish As String = CStr(UserToAdmonish.Attribute("UserAdmonish"))
          Dim ListUser() As String = Split(UserAdmonish, ",")
          Dim CurrentUser As User = Authentication.CurrentUser(Nothing)
          If Array.IndexOf(ListUser, CurrentUser.Username) = -1 Then
            If Not String.IsNullOrEmpty(UserAdmonish) Then
              UserAdmonish &= ","
            End If
            UserAdmonish &= CurrentUser.Username
            UserToAdmonish.Attribute("UserAdmonish") = UserAdmonish
            If UserToAdmonish.Role(Setting.Name) <= Authentication.User.RoleType.Senior Then
              Dim Admonition As String = CStr(UserToAdmonish.Attribute("Admonition"))
              Dim LastAmmonition As Date
              Dim Value As Integer = 0
              If Not String.IsNullOrEmpty(Admonition) Then
                Dim Data() As String = Split(Admonition, " ")
              ...
              End If
              Value += AddValue
              Admonition = Now.ToUniversalTime().ToOADate.ToString(Globalization.CultureInfo.InvariantCulture) & " " & Value
              UserToAdmonish.Attribute("Admonition") = Admonition
              UserToAdmonish.Save()

              'Log This operation
              Extension.Log("Admonish", 1000, CurrentUser.Username, AddValue, UserName)
            End If
          End If
        End If
      End If
    ...
    Public Sub RequireCensore(ByVal Setting As SubSite, ByVal ForumId As Integer, ByVal TopicID As Integer, ByVal ReplyID As Integer)
      ...
      End If
      Dim Reply As Reply
      Reply = ForumManager.Reply.Load(ForumId, TopicID, ReplyID)
    ...
    Public Sub RequireSetCloseTopic(ByVal Setting As SubSite, ByVal ForumId As Integer, ByVal TopicID As Integer, Close As Boolean)
      If CurrentUser(Nothing).Role(Setting.Name) >= Authentication.User.RoleType.AdministratorJunior Then
        'close topic
    ...
    Public Function AbleToAdmonish(ByVal Setting As SubSite) As Boolean
      Return CurrentUser(Nothing).Role(Setting.Name) > Authentication.User.RoleType.Senior
    End Function
    ...
    Function Paginate(ByVal Setting As SubSite, ByVal TotalPage As Integer, ByVal CurrentPage As Integer, ByVal Action As ActionType, ByVal ForumId As Integer, Optional ByVal TopicID As Integer = 0, Optional ByVal FindText As String = Nothing, Optional ByVal SubCategory As Integer = 0) As HtmlGenericControl
      If TotalPage > 1 Then
        Dim Ctrl As New HtmlGenericControl("nav")
        Dim Href As String
        If CBool(TopicID) Then
          Href = Link(Action, Setting.Name, False, Nothing, ForumId, TopicID)
        Else
          Href = Link(Action, Setting.Name, False, Nothing, ForumId, 0, 0, 0, Nothing, FindText, False, Nothing, SubCategory)
        End If
        If TotalPage > Setup.Forum.NPaginating Then
          Dim Icon As New WebControls.HyperLink
          Icon.Text = CStr(ChrW(8678)) 'Left
        ...
        If TotalPage > Setup.Forum.NPaginating Then
          Ctrl.Controls.Add(New LiteralControl(" "))
          Dim Icon As New WebControls.HyperLink
          Icon.Text = CStr(ChrW(8680)) 'Right
    ...
    Public Function AbleToModifyReply(ByVal Setting As SubSite, ByVal Author As String, ByVal Create As Date, Optional ByVal AuthorAccount As String = Nothing) As Boolean
      ...
      If Not String.IsNullOrEmpty(Author) AndAlso AuthorAccount Is Nothing Then
        C2 = StrComp(Author, CurrentUser.Username, CompareMethod.Text) = 0
      End If
      Dim C3 As Boolean = DateDiff(DateInterval.Day, Create, Now) < 10 OrElse CurrentUser.Role(Setting.Name) >= Authentication.User.RoleType.AdministratorJunior
      If C1 OrElse (C2 AndAlso C3) Then
        Return True
      Else
        Return False
      End If
    ...
    Public Sub AddDateAndTime(ByVal Control As Control, ByVal DateValue As Date)
      Dim PersonalizedDate As RenderMethod = Sub(Writer As System.Web.UI.HtmlTextWriter, Container As System.Web.UI.Control)
                                               Dim CurrentUser As Authentication.User = Authentication.CurrentUser(Nothing)
                                               Dim DateValueUser As Date = Date.MinValue
                                               ...
                                               Dim TimeControl As New HtmlGenericControl("time")
                                               TimeControl.Attributes.Add("datetime", DateValueUser.ToString("s"))
                                               TimeControl.InnerText = " " & DateValueUser.ToString(Setting.DateTimeFormat(), Setting.Culture())
                                               If DateValueUser > CurrentUser.LastAccess AndAlso CurrentUser.LastAccess <> System.DateTime.MinValue Then
                                                 TimeControl.Attributes.Add("class", "Evidence")
                                               End If
                                               TimeControl.RenderControl(Writer)
                                             End Sub
      Dim PlaceHolder As New PlaceHolder
          ...
          If Topic.LastReplyNotCensoredId(False, Nothing) = 1 Then
            If Topic.FirstReply().Author IsNot Nothing AndAlso Topic.FirstReply().AuthorAccount Is Nothing Then
              ...
              If DateDiff(DateInterval.Hour, Author.FirstMessageInForum, Now.ToUniversalTime()) < Setup.Security.HoursEvaluationTimeNewUser Then
                If StrComp(Topic.FirstReply().Author, CurrentUser(Nothing).Username, CompareMethod.Text) <> 0 Then
                  Return True
                End If
              End If
            End If
          ...
          If DateDiff(DateInterval.Hour, Reply.Created, Now.ToUniversalTime()) < Setup.Security.HoursEvaluationTimeNewUser Then
            If Reply.AuthorAccount Is Nothing Then
              ...
              If Author IsNot Nothing Then
                If DateDiff(DateInterval.Hour, Author.FirstMessageInForum, Now.ToUniversalTime()) < Setup.Security.HoursEvaluationTimeNewUser Then
                  If CheckAuthorCurrentUser = False OrElse StrComp(Author.Username, CurrentUser(Nothing).Username, CompareMethod.Text) <> 0 Then
                    'Enabled show topic of new user if administrator reply in this topic
                    If Topic Is Nothing Then
                      Topic = ForumManager.Topic.Load(Reply.ForumId, Reply.TopicID)
                    End If
    ...
    Function EnableShowHidden() As Boolean
      Return CurrentUser(Nothing).Role(CurrentSubSiteName()) >= User.RoleType.AdministratorJunior
    End Function

    Public Sub FeedRssForum(ByRef RssFeed As FeedRSSManager.rss, ByVal Setting As SubSite, ByVal EnableShowHidden As Boolean, ByVal ForumId As Integer, Optional ByVal TopicId As Integer = 0, Optional ByVal SubCategory As Integer = -1)
      If CBool(TopicId) Then
        Dim Topic As Topic = ForumManager.Topic.Load(ForumId, TopicId)
        If Topic IsNot Nothing Then
          ...
          If FirstReply IsNot Nothing Then
            ...
            For Each Reply As Reply In Topic.Replies(False)
              ...
              RssItem.link = ForumManager.Link(ForumManager.ActionType.Reply, Setting.Name, True, Nothing, ForumId, FirstReply.TopicID, IfInt(Reply.ID <> Topic.LastReplyId(EnableShowHidden), Reply.ID, 0))
              RssItem.title = FirstReply.Title
              RssItem.description = Reply.Text
              RssItem.pubDate = Format(Reply.Created, "R")
              If Not String.IsNullOrEmpty(Reply.Photo) Then
        ...
        End If
      Else
        Dim Topics As System.Collections.Generic.List(Of ForumManager.Topic) = AllTopics(ForumId, False, ActionType.Show, Nothing, SubCategory)
        'If Not Topics Is Nothing Then
        If Topics IsNot Nothing AndAlso Topics.Count <> 0 Then
          Dim Added As Integer = 0
          Dim CurrentUserPreferences As UserPreferences = New UserPreferences(CurrentUser(Nothing))
          For Each Topic As Topic In Topics
            Dim FirstReply As Reply = Topic.FirstReply()
            If FirstReply IsNot Nothing Then
              Dim rssItem As New rssChannelItem
              rssItem.link = ForumManager.Link(ForumManager.ActionType.Show, Setting.Name, True, Nothing, ForumId, Topic.ID, 0, Topic.TotalPages(CurrentUserPreferences.ShowCensored))
              rssItem.title = FirstReply.Title
              rssItem.description = FirstReply.Text
              rssItem.pubDate = Format(FirstReply.Created, "R")
              If Not String.IsNullOrEmpty(FirstReply.Photo) Then
    ...
    Function AllTopics(ByVal ForumId As Integer, ByVal ShowCensored As Boolean, Optional ByVal Action As ActionType = ActionType.Show, Optional ByVal FindText As String = Nothing, Optional ByVal SubCategory As Integer = -1) As System.Collections.Generic.List(Of ForumManager.Topic)
            ...
            Threading.Tasks.Parallel.ForEach(TopicsCache.Values, Sub(Topic)
                                                                   ...
                                                                   If Topic IsNot Nothing Then
                                                                     If TopicMomentarilyHidden(Topic) = False OrElse ShowHidden Then
                                                                       Dim Add As Boolean
                                                                       If Action = ActionType.FindAllWords OrElse Action = ActionType.FindOneWords OrElse Action = ActionType.FindPhrase Then
      ...
      SortTopic(ShowCensored, ShowHidden, CurrentUser(Nothing).Username, Topics)
      ...
      For Each Reply As Reply In Topic.AllReplies(ForumId, TopicID, FindInCensoredReply, ShowHidden)
        ...
        Select Case Action
          ...
          Case ActionType.FindAllWords
            Dim Back As String = "" 'Need for c# conversion
            Return CBool(FindWord(1, Source, Words, TextType.Text, Nothing, CompareMethod.Text, Back))
    ...
    Class ForumSetting
      ...
      Function Name() As String
        If Not String.IsNullOrEmpty(NameInSubsite) Then
      ...
      Function Description() As String
        If Not String.IsNullOrEmpty(DescriptionInSubsite) Then
    ...
    Class Topic
      ...
      Public Sub New(ByVal ForumId As Integer, ByVal SubCategory As Integer, ByVal Author As String, ByVal Title As String, ByVal Keywords As String, ByVal TextHtml As String, Optional ByVal Photo As String = Nothing, Optional ByVal Video As String = Nothing, Optional ByVal SaveNow As Boolean = True, Optional ByVal Reference As String = Nothing, Optional ByVal AuthorAccoun As String = Nothing, Optional ByVal Id As Integer = 0, Optional ByVal PollDetection As Boolean = True, Optional Created As Date = Nothing)
        ...
        Else
          ...
          Dim SaveStructure As Boolean = False
        ...
        Dim FirstReply As Reply = New Reply(ForumId, Me.ID, Author, Title, Keywords, TextHtml, Nothing, 0, Photo, Video, False, AuthorAccoun, PollDetection, Created)
      End Sub
      ...
      Shared Function Exists(ByVal ForumId As Integer, ByVal TopicID As Integer) As Boolean
        ...
        If Root.Exists = True AndAlso (Root.Attributes And IO.FileAttributes.Hidden) = 0 Then
          Return True
        End If
        Return False
      End Function
      ...
      Private Sub CheckNoHideLastReply(ByVal NoCensored As Boolean)
        ...
        If DateDiff(DateInterval.Hour, ReplyCreatedCreated, Now.ToUniversalTime()) < Setup.Security.HoursEvaluationTimeNewUser Then
          LastReply = ForumManager.Reply.Load(ForumId, ID, IfInt(NoCensored, m_LastReplyNotCensoredId, m_LastReplyId))
        End If
      ...
      Function FirstReply() As Reply
        Return ForumManager.Reply.Load(ForumId, ID, FirstReplyId)
      End Function
      ...
      Function LastReply(ByVal ShowCensored As Boolean, ByVal ShowHiden As Boolean, Optional ByVal User As String = Nothing, Optional ByVal RecursiveCount As Integer = 0) As Reply
        If ShowCensored Then
          If LastNoHideReplyCheck = CheckNoHide.Visible OrElse ShowHiden OrElse (LastReplyAuthorAccount Is Nothing AndAlso User = LastReplyAuthor) Then
            Return ForumManager.Reply.Load(ForumId, ID, m_LastReplyId)
          ElseIf DateDiff(DateInterval.Hour, LastReplyAuthorFirstMessage, Now.ToUniversalTime()) >= Setup.Security.HoursEvaluationTimeNewUser Then
            ...
            If RecursiveCount >= 2 Then
              ...
              Extension.Log("ErrorRecursive", 1000, "Function LastReply (All)", User, CStr(Me.ForumId), CStr(LastNoHideReplyCheck), CStr(Me.m_LastReplyId), CStr(Me.ID), CStr(m_LastNoHideReplyId), CStr(LastNoHideReplyNotCensoredCheck), CStr(Me.m_LastNoHideReplyNotCensoredId), CStr(Me.m_LastNoHideReplyNotCensoredId), CStr(DateDiff(DateInterval.Hour, m_LastReplyCreated, Now.ToUniversalTime())))
              Return ForumManager.Reply.Load(ForumId, ID, m_LastNoHideReplyId)
        ...
        Else
          If LastNoHideReplyNotCensoredCheck = CheckNoHide.Visible OrElse ShowHiden OrElse (LastReplyNotCensoredAuthorAccount Is Nothing AndAlso User = LastReplyNotCensoredAuthor) Then
            Return ForumManager.Reply.Load(ForumId, ID, m_LastReplyNotCensoredId)
          ...
          ElseIf DateDiff(DateInterval.Hour, LastReplyNotCensoredAuthorFirstMessage, Now.ToUniversalTime()) >= Setup.Security.HoursEvaluationTimeNewUser Then
            ...
            If RecursiveCount >= 2 Then
              ...
              Extension.Log("ErrorRecursive", 1000, "Function LastReply (noCensore)", User, CStr(Me.ForumId), CStr(LastNoHideReplyCheck), CStr(Me.m_LastReplyId), CStr(Me.ID), CStr(m_LastNoHideReplyId), CStr(LastNoHideReplyNotCensoredCheck), CStr(Me.m_LastNoHideReplyNotCensoredId), CStr(Me.m_LastNoHideReplyNotCensoredId), CStr(DateDiff(DateInterval.Hour, m_LastReplyNotCensoredCreated, Now.ToUniversalTime())))
              Return ForumManager.Reply.Load(ForumId, ID, m_LastNoHideReplyNotCensoredId)
      ...
      Public Function Replies(ByVal ShowCensored As Boolean) As Collections.Generic.List(Of Reply)
        Dim Root As String = FileManager.MapPath(ForumSubDirectory & "\" & ForumId & "\" & ID)
        Dim Dir As New System.IO.DirectoryInfo(Root)
        If Dir.Exists Then
          ...
          For Each File As System.IO.FileSystemInfo In Files
            Dim ReplyID As Integer = ValInt(File.Name)
            Dim Reply As Reply = ForumManager.Reply.Load(ForumId, ID, ReplyID)
      ...
      Public Shared Function AllReplies(ByVal ForumId As Integer, ByVal TopicID As Integer, ByVal ShowCensored As Boolean, ByVal ShowHiden As Boolean) As Reply()
        Dim Root As String = FileManager.MapPath(ForumSubDirectory & "\" & ForumId & "\" & TopicID)
        'Dim dir As New System.IO.DirectoryInfo(Root)
        If System.IO.Directory.Exists(Root) Then
          ...
          Dim LastReply As Integer = 0
          Dim Topic As Topic = ForumManager.Topic.Load(ForumId, TopicID)
          For Each File As String In Documents
            replyID = ValInt(FileManager.GetFileName(File))
            LastReply = replyID
            Dim Reply As Reply = ForumManager.Reply.Load(ForumId, TopicID, replyID)
            If ReplyMomentarilyHidden(Reply, True, Topic) = False OrElse EnableShowHidden() Then
              If ShowCensored OrElse Reply.Censored = False Then
          ...
          If Topic.LastReplyId(True) <> LastReply Then
            Extension.Log("ErrorLastReplyId", 1000, Topic.ForumId, Topic.ID, Topic.LastReplyId(True), LastReply)
            ...
            If Topic IsNot Nothing Then
              If Topic.LastReplyId(True) <> LastReply Then
                ...
                Extension.Log("ErrorLastReplyId", 1000, Topic.ForumId, Topic.ID, Topic.LastReplyId(True), LastReply, "Persist")
      ...
      Private Shared Function StartupServerNotification() As Boolean
        Pipeline.AddActionForNotification(GetType(Topic), New WebApplication.Pipeline.NotifyChangementDelegate(AddressOf NotifyTopicHasChangedFromAnotherServer))
        Return True
      End Function
      ...
      Shared Sub SaveCache()
        If CacheLoaded Then
          Try
            ...
            Next
          Catch ex As Exception
            Extension.Log("SpecificError", 1000, "SaveCache", ex.Source, ex.Message, ex.StackTrace)
          End Try
        End If
      End Sub
      ...
      Shared Sub LoadCache()
        For Each Forum As ForumManager.Forum In ForumManager.AllForums(False)
          ...
          If Infos IsNot Nothing Then
            ...
            If Infos.Length <> Forum.TotalTopics Then
              Dirs = Forum.GetAllTopicsId()
            End If

            For Each InfoId As TopicInfoId In Infos
              If Dirs Is Nothing OrElse Array.IndexOf(Dirs, InfoId.Id) <> -1 Then
                Dim IstanceOfTopic As Topic
                IstanceOfTopic = New Topic
                IstanceOfTopic.ForumId = Forum.Id
                IstanceOfTopic.ID = InfoId.Id
                Try
                  IstanceOfTopic.LoadInfo(InfoId.Info)
                  SyncLock Collection
                    Try
                      If Not Topics.ContainsKey(IstanceOfTopic.ID) Then
                        Topics.Add(IstanceOfTopic.ID, IstanceOfTopic)
                      End If
                    Catch ex As Exception
                    End Try
                  End SyncLock
          ...
          Try
            ...
            Else
              If Exists(ForumId, TopicID) Then
                Dim TopicIstance As New Topic
                TopicIstance.LoadFromFile(ForumId, TopicID)
                If TopicIstance.FirstReplyId = 0 Then
                  'This topic is empty! Must be deleted!
                  Topic.Delete(ForumId, TopicID)
                Else
                  Try
                    AllTopics.Add(TopicID, TopicIstance)
            ...
            End If
          Catch ex As Exception
            Extension.Log("ErrorLoadTopic", 100, ForumId, TopicID, ex.Message, ex.StackTrace)
          End Try
        End SyncLock
        Return Nothing
      End Function
      ...
      Shared Sub Delete(ByVal ForumId As Integer, ByVal TopicID As Integer)
        ...
        If AllTopics IsNot Nothing Then
          If AllTopics.ContainsKey(TopicID) Then
            'Log This operation
            Dim TopicToDelete As Topic = AllTopics(TopicID)
            Extension.Log("DeleteTopic", 1000, CurrentUser(Nothing).Username, ForumId, TopicID, TopicToDelete.Title())
            SyncLock Collection
              AllTopics.Remove(TopicID)
            End SyncLock
            ...
            Dim Forum As Forum = CType(ForumManager.Forum.Load.GetItem(ForumId), ForumManager.Forum)
            Forum.TotalTopics -= 1
            Dim SubCategory As ForumManager.ForumStructure.Category.Subcategory = Forum.SubCategory(TopicToDelete.SubCategory)
            Dim SaveStructure As Boolean = False
            If SubCategory IsNot Nothing Then
              If Not TopicToDelete.Censored Then
                SaveStructure = True
                SubCategory.TotalTopics -= 1
              End If
            End If
            Forum.Save(SaveStructure)
            TopicToDelete.Finalize()
          End If
        End If
      End Sub
      ...
      Public Sub UpdateLastReplyNotCensored(Optional ByVal Save As Boolean = True)
        ...
        If NewLastReplyNotCensored Is Nothing Then
          ...
          m_LastReplyNotCensoredCreated = Date.MinValue
          If Save Then
            Me.SaveInfo()
          End If
      ...
      Private Sub RepairInfoTopic()
        ...
        ElseIf m_FirstReplyId = 0 Then
          Dim ReplyToRepair As Reply = ForumManager.Reply.Load(ForumId, ID, 1)
          If ReplyToRepair IsNot Nothing Then
            m_FirstReplyId = 1
            Me.Created = ReplyToRepair.Created
            RepairDescription = "m_FirstReplyId = 0"
          End If
        End If

        If m_LastReplyNotCensoredId = 0 AndAlso Censored = False Then
          Censored = True
        ...
        If Not ForumForInsideUse(ForumId) Then
          If String.IsNullOrEmpty(Keywords) Then
            Dim FirstReply As Reply = ForumManager.Reply.Load(ForumId, ID, FirstReplyId)
            If FirstReply IsNot Nothing Then
              Me.Censored = FirstReply.Censored
              If Not String.IsNullOrEmpty(Trim(FirstReply.Keywords)) Then
                Keywords = FirstReply.Keywords
              ElseIf Not String.IsNullOrEmpty(FirstReply.Title) Then
        ...
        If RepairDescription IsNot Nothing Then
          '=========================remove this bloch
          Extension.Log("RepairInfoTopic", 1000, ForumId, ID, RepairDescription)
          '==========================================
          SaveInfo()
        End If
      End Sub
      ...
      Private Function NameFileTopicInfo(ForumId As Integer, TopicID As Integer) As String
        Return FileManager.MapPath(ForumSubDirectory & "\" & ForumId & "\" & ID & "\topic.xml")
      End Function
    ...
    Class Reply
        ...
        Set(ByVal value As Integer)
          ...
          If Me.Censored = False Then
            ...
            If ReservedForums.PrivateMessage = ForumId Then
              Proprietary = Authentication.User.Load(ForumManager.Reply.Load(ForumId, TopicID, ID).Title)
            End If

            If AbleToCensore(Nothing, Proprietary) OrElse (Me.Author = CurrentUser(Nothing).Username AndAlso Me.AuthorAccount Is Nothing) OrElse Me.NCensoreRequireds > Minimum Then
              Censored = True
              Me.Censored = True
              'Censore reply at Censored message
              For Each ReplyQuote As Reply In ForumManager.Topic.Load(ForumId, TopicID).Replies(False)
      ...
      Private Sub UpdateText()
        If Me.QuoteID <> 0 AndAlso Not String.IsNullOrEmpty(TextQuote) Then
          Dim QuoteAuthor As String = ForumManager.Reply.Load(Me.ForumId, Me.TopicID, QuoteID).Author
          Me.m_Text = "<blockquote><b>" & QuoteAuthor & ": </b><br />" & TextQuote & "</blockquote>"
          Me.m_Text &= Me.m_TextReply
        ElseIf Not String.IsNullOrEmpty(m_TextReply) Then
          Me.m_Text = Me.m_TextReply
        End If
      End Sub
      ...
      Sub Save(Optional ByVal ForceID As Integer = 0, Optional ByVal PollDetection As Boolean = True)
        If CBool(TopicID) Then
          Dim Meta As Common.MetaTags = New Common.MetaTags(Title, "", Keywords, Author)
          Meta.AddMetaTag("create", DateToText(Created))

          If Modified <> New Date Then
            Meta.AddMetaTag("modified", DateToText(Modified))
          End If

          If Not String.IsNullOrEmpty(AuthorAccount) Then
            Meta.AddMetaTag("AuthorAccount", AuthorAccount)
          End If

          If CBool(Theme) Then
            Meta.AddMetaTag("theme", CStr(Theme))
          End If

          If Not String.IsNullOrEmpty(Photo) Then
            Meta.AddMetaTag("photo", Photo)
          End If

          If Not String.IsNullOrEmpty(Video) Then
            Meta.AddMetaTag("Video", Video)
          End If

          If IsCensored Then
            Meta.AddMetaTag("Censored", CStr(IsCensored))
          End If

          If CBool(RequestCensorship) Then
            Meta.AddMetaTag("Censorerequireds", CStr(RequestCensorship))
          End If

          Dim Body As String
          If CBool(QuoteID) Then
            Meta.AddMetaTag("quote", CStr(QuoteID))
          End If
          Body = Me.Text
          ...
          If Poll IsNot Nothing OrElse Me.IsPoll Then
            Meta.AddMetaTag("poll", VotesToPollString(Poll))
          End If

          Dim Document As HtmlDocument = New HtmlDocument(Meta, Body)
          ...
          ElseIf m_id = 0 Then
            ...
            If AuthorAccount Is Nothing Then
              Dim AuthorOfReply As Authentication.User = Authentication.User.Load(Me.Author)
              If Not Me.Text Is Nothing Then
                Dim ByteWrite As Long
                Try
                  ByteWrite = CLng(AuthorOfReply.Attribute(NameAttribute & Me.ForumId))
                Catch ex As Exception
                  ByteWrite = 0
                End Try
                ByteWrite += Me.TextReply.Length
                AuthorOfReply.Attribute(NameAttribute & Me.ForumId) = ByteWrite
                If AuthorOfReply.FirstMessageInForum = Date.MinValue Then
                  AuthorOfReply.FirstMessageInForum = Created
                End If
                AuthorOfReply.Save()
              End If
            End If
          ...
          'Abjust LastReply attribute in Topic
          Dim Topic As Topic = ForumManager.Topic.Load(Me.ForumId, Me.TopicID)

          Dim IsNewPost As Boolean = False
          ...
          Reply.Cache.SetItem(Key, Me)

          Dim SaveTopic As Boolean = False
          If IsNewPost Then
            Topic.IncReplies(Me)
            SaveTopic = True
          End If
          ...
          If SaveTopic Then
            Topic.Save()
            Dim t As Topic = ForumManager.Topic.Load(Me.ForumId, Me.TopicID)
          End If
      ...
      Shared Function QueryString(ByVal SubSite As SubSite, ByVal ForumId As Integer, ByVal TopicId As Integer, ByVal PageNumber As Integer) As String
        Return Link(ActionType.Show, SubSite.Name, False, Nothing, ForumId, TopicId, 0, PageNumber)
      End Function
      ...
      Public Shared Function Load(ByVal ForumId As Integer, ByVal TopicID As Integer, ByVal ID As Integer) As Reply
        Return CType(ForumManager.Reply.Cache.GetItem(KeyCache(ForumId, TopicID, ID)), Reply)
      End Function
    ...
    Sub SendEmailNotification(Setting As SubSite, Topic As Topic)
      If Not String.IsNullOrEmpty(Setup.Email.Email.Email) Then
        ...
        Show.NavigateUrl = Link(ActionType.Show, Nothing, True, "PageBottom", Topic.ForumId, Topic.ID)
        Body &= ControlToText(Show) & "<br />"
        Dim Subject As String = Nothing
        If Topic.SubCategory > -1 Then
          Subject = "[" & Topic.SubCategory & "] "
        End If
        SendEmail(Subject & Topic.Title, Body, Setup.Email.Email.Email, True, True, False, Configuration.EmailSender.Administrator)
      End If
    End Sub
    ...
    Class Subscription
          ...
          If CBool(Datas.Length) Then
            For Each Record As String In Datas
              If Not String.IsNullOrEmpty(Record) Then
                ...
                Try
                  Dim IsSubscript As TypeSubscription
                  IsSubscript = CType(CInt(Field(1)), TypeSubscription)
                  Dim Suspend As Boolean
                  Suspend = CBool(Field(2))
                  Dim UserSubscription As New UserSubscription(Field(0), IsSubscript, Suspend, Field(3), Field(4))
      ...
      Property IsSubscript(ByVal UserName As String, Optional ByVal ServerName As String = Nothing, Optional ByVal Setting As String = Nothing) As TypeSubscription
        ...
        Set(ByVal Value As TypeSubscription)
          ...
          Else
            UserSubscription = New ForumManager.Subscription.UserSubscription(UserName, Value, False, ServerName, Setting)
      ...
      Sub SendNotification(ByVal User As User, ByVal Title As String)
        If User IsNot Nothing AndAlso Not String.IsNullOrEmpty(User.Email) Then
          ...
          If UserSubscription IsNot Nothing AndAlso UserSubscription.IsSubscript = TypeSubscription.IsTrue AndAlso Not UserSubscription.Suspend = True Then
            ...
            Unsubscribe.NavigateUrl = Link(ActionType.Unsubscribe, UserSubscription.SubSite, True, Nothing, ForumId, TopicID, 0, 0, User.Username)
            Body &= ControlToText(Show) & "<br />"
            Body &= ControlToText(Unsubscribe) & "<br />"
            SendEmail(Subject, Body, User.Email, True, True, False, Configuration.EmailSender.Administrator)
          End If
        End If
      End Sub
    ...
    Class UserPreferences
      ...
      End Sub
      Sub Save(ByVal RelativeToUser As Authentication.User)
        Dim Bol As String = _
        ...
        Abs(ShowAvatars)
        RelativeToUser.Attribute("ForumSetting") = Bol
        RelativeToUser.Save()
      End Sub
      Sub Load(ByVal RelativeToUser As Authentication.User)
        Dim Bl As String = CStr(RelativeToUser.Attribute("ForumSetting"))
        ...
        Else
          Try
            ...
            If RelativeToUser.GeneralRole <= Authentication.User.RoleType.Excluded Then
    ...
    Function Normalize(ByVal Text As String) As String
      ...
      Dim OC As Char = Char.MinValue
      Dim LC As Char
      Dim Repetition As Integer = 0
      For Each C As Char In Text
        LC = Char.ToLower(C)
        If C.Equals(OC) AndAlso Not Char.IsDigit(C) Then
          Repetition += 1
        Else
          Repetition = 0
        End If
    ...
    Sub AddForumPhotoAlbum(ByVal Forum As ForumManager.Forum, ByVal Language As LanguageManager.Language)
      Dim RootPhotoAlbum As PhotoAlbum = CType(PhotoManager.PhotoAlbum.Load.GetItem("forum"), PhotoManager.PhotoAlbum)
      If RootPhotoAlbum Is Nothing Then
        RootPhotoAlbum = New PhotoManager.PhotoAlbum
      ...
      Dim ForumAlbum As PhotoManager.PhotoAlbum = CType(PhotoManager.PhotoAlbum.Load.GetItem(ForumName), PhotoManager.PhotoAlbum)
      If ForumAlbum Is Nothing Then
        ForumAlbum = New PhotoManager.PhotoAlbum
        ForumAlbum.Name = ForumName
        ForumAlbum.IsRoot = True
        ForumAlbum.Editable = PhotoManager.EditablePermission.None
        ForumAlbum.AddPermitted = PhotoManager.Permission.Permitted
        ForumAlbum.Deletable = PhotoManager.Permission.Author
        ForumAlbum.SubPhotoAlbumsNotCreatable = True
        ForumAlbum.Title(Language) = Phrase(Language, 61, 103)
        ForumAlbum.Save()
        Forum.Save()
      End If
    End Sub
    ...
    Public Function ShowTopic(ByVal Setting As SubSite, ByVal ForumId As Integer, ByVal TopicID As Integer, ByVal CurrentPage As Integer, ByVal ShowCensored As Boolean, ByVal ShowAvatars As Boolean, Optional ByVal OperationsColumn As Boolean = True, Optional ByVal Page As Page = Nothing, Optional ByVal Action As ActionType = ActionType.Show, Optional ByVal Jump As String = Nothing, Optional ByVal AddButtons As Boolean = False, Optional ByVal LinksCorrelated As Control = Nothing, Optional ByVal AddTitle As Boolean = True, Optional ByVal SetMetaRevisitAfter As Boolean = False) As Control
      ...
      If Topic Is Nothing OrElse Topic.FirstReply() Is Nothing Then
        ...
        Else
          HttpContext.Current.Response.RedirectPermanent(ForumManager.Link(ForumManager.ActionType.Show, Setting.Name, False, Nothing, ForumId), True)
        End If
      End If
      ...
      If Topic IsNot Nothing Then
        ...
        If Topic.Censored AndAlso Not ShowCensored Then
          ...
          ElseIf Not ForumForInsideUse(ForumId) Then
            HttpContext.Current.Response.Redirect(ForumManager.Link(ForumManager.ActionType.Show, Setting.Name, False, Nothing, ForumId), True)
          End If
        End If
        ...
        If MasterPage IsNot Nothing Then
          Select Case ForumId
            ...
            Case ReservedForums.ArchiveComment
              ...
              If APL.Length = 3 Then
                Dim Html As String = ReadAll(MenuManager.PageNameFile(CInt(APL(0)), CInt(APL(1)), CType(CInt(APL(2)), WebApplication.LanguageManager.Language)))
            ...
            Case Else
              ...
              If Not String.IsNullOrEmpty(Topic.Keywords) Then
        ...
        For EachC As Integer = c.GetLowerBound(0) To c.GetUpperBound(0)
          ...
          Select Case NC
            ...
            Case 1  'Message
              'Set RSS link
              TitleRow.Cells(EachC).Controls.Add(RssButton(ForumManager.Link(ForumManager.ActionType.ShowRSS, Setting.Name, False, Nothing, ForumId, TopicID)))
              'Set PDF link
              ...
              If Page IsNot Nothing Then
                ...
                Meta.Href = ForumManager.Link(ForumManager.ActionType.ShowRSS, Setting.Name, False, Nothing, ForumId, TopicID)
                Page.Header.Controls.Add(Meta)
              End If
              ...
              'Add Arrow "go bottom"
              TitleRow.Cells(EachC).Controls.Add(IconUnicode(IconName.ArrowDown, False, Phrase(Setting.Language, 3051), "#PageBottom"))
          ...
          If DateDiff(DateInterval.Hour, Topic.LastReplyNotCensoredCreate(True).ToUniversalTime(), Now.ToUniversalTime()) > Setup.Security.HoursEvaluationTimeNewUser Then
            If ShowCensored Then
              If Topic.NReplies <> Replies.GetUpperBound(0) + 1 Then
                Extension.Log("ErrorNRepliesInTopic", 1000, "NReplies", Topic.ForumId, Topic.ID, Topic.NReplies, Replies.GetUpperBound(0) + 1)
                Topic.NReplies = Replies.GetUpperBound(0) + 1
                Topic.Save()
              End If
            Else
              If Topic.NRepliesNotCensored <> Replies.GetUpperBound(0) + 1 Then
                Extension.Log("ErrorNRepliesInTopic", 1000, "NRepliesNotCensored", Topic.ForumId, Topic.ID, Topic.NRepliesNotCensored, Replies.GetUpperBound(0) + 1)
            ...
            For EachC As Integer = c.GetLowerBound(0) To c.GetUpperBound(0)
              ...
              Select Case NC
                Case 0  'Author
                  ...
                  Dim InfoUser As RenderMethod = Sub(Writer As System.Web.UI.HtmlTextWriter, Container As System.Web.UI.Control)
                                                   If Reply.AuthorAccount Is Nothing Then
                                                     Components.QuickInfoUser(Config.CurrentSetting(), Author, CurrentForum).RenderControl(Writer)
                                                   Else
                                                     Components.QuickInfoUserExternalAccount(Reply.Author, Reply.AuthorAccount).RenderControl(Writer)
                                                   End If
                                                 End Sub
                  Dim PlaceHolder As New PlaceHolder
                  PlaceHolder.SetRenderMethodDelegate(InfoUser)
                  ...
                  If Setting.SEO.LinkToWebsitesOfUsersInTheForums <> SubSite.SeoFunctions.TypeLink.No AndAlso Author IsNot Nothing Then
                    If Not String.IsNullOrEmpty(Author.URL) AndAlso (Author.URL.StartsWith("http://") OrElse Author.URL.StartsWith("https://")) Then
                      Row.Cells(EachC).Controls.Add(BR)
                      Dim Link As New HyperLink
                      Link.CssClass = "url"
                      ...
                      Select Case Setting.SEO.LinkToWebsitesOfUsersInTheForums
                        ...
                        Case SubSite.SeoFunctions.TypeLink.YesWithTheTitle
                          ...
                          If String.IsNullOrEmpty(TitleOfWebSite) OrElse CStr(Author.Attribute("UrlOfWebSite")) <> Author.URL Then
                            Try
                              TitleOfWebSite = New Common.MetaTags(ReadHtmlFromWeb(Author.URL)).Title
                            Catch ex As Exception
                            End Try
                            If String.IsNullOrEmpty(TitleOfWebSite) Then
                ...
                Case 1  'Message
                  ...
                  'Add Photo
                  Dim MyPhoto As PhotoManager.Photo = PhotoManager.Photo.Load(Reply.Photo)
                  If MyPhoto IsNot Nothing Then
                    Dim Box As HtmlControl = CType(MyPhoto.ControlThumbnail(Setting, False, Config.SizeImagePreview()), HtmlControl)
                    Box.Attributes.Add("class", "BoxElement PhotoUnpair")
                    Cell.Controls.Add(Box)
                  End If

                  'Add video
                  If Not String.IsNullOrEmpty(Reply.Video) Then
                  ...
                  If Setup.Performance.MultyThread Then
                    ...
                    Dim www As ContextualLink.AddWWW = ContextualLink.AddWWW.Auto
                    ...
                    Dim NewThread = New Threading.Thread(Sub()
                                                           GetReplyText(Cell, ForumId, TopicID, Reply, Setting, Archive, ReplyNumber, www)
                                                         End Sub)
                  ...
                  Else
                    'Mono Thread
                    GetReplyText(Cell, ForumId, TopicID, Reply, Setting, Archive, NReply)
                  End If

                  'Add extra info
                  If Reply.Censored Then
                    Row.Cells(EachC).Controls.Add(BR)
                    Row.Cells(EachC).Controls.Add(Components.Message(Setting, 2009, 0, Nothing, MessageType.Normal))
                  ElseIf ReplyMomentarilyHidden(Reply, True, Topic) Then
                    'Message momentarily hidden
                    Row.Cells(EachC).Controls.Add(BR)
                    Row.Cells(EachC).Controls.Add(Components.Message(Setting, 2008, 0, Nothing, MessageType.Normal))
                  End If
                Case 2 'Operations
                  ...
                  'Reply with quote
                  'Dim UrlHref As String = ForumManager.Link(ForumManager.ActionType.Reply, Setting.Name, False, Jump, ForumId, TopicID, ReplyID) 'lo ho messo qui per vedere come mai in c# da errore
                  Dim Quote As Control = Components.IconUnicode(IconName.LeftPointingIndex, False, Phrase(Setting.Language, 3071), ForumManager.Link(ForumManager.ActionType.Reply, Setting.Name, False, Jump, ForumId, TopicID, ReplyID))
                  Row.Cells(EachC).Controls.Add(Quote)

                  If AbleToModifyReply(Setting, Reply.Author, Create, Reply.AuthorAccount) Then
                    'Modify message
                    Dim Edit As Control = Components.IconUnicode(IconName.WritingHand, False, Phrase(Setting.Language, 74), Link(ActionType.Modify, Setting.Name, False, Jump, ForumId, TopicID, ReplyID))
                  ...
                  If AbleToCensore(Setting, PrivateTopicOfUser) OrElse (CBool(Stars(CurrentUser)) AndAlso CurrentForum.Type <> Forum.TypeOfForum.TicketSystem) Then
                    'Require Censore
                    Dim NavigateUrl As String = Href(Setting.Name, False, "messenger.aspx", MessengerManager.QueryStringParameters(, MessengerManager.Action.RequireTopicCensore, False, Nothing, ForumId, TopicID, ReplyID))
                    Dim Censore As Control = Components.IconUnicode(IconName.ScissorBlack, False, Phrase(Setting.Language, 3075), NavigateUrl)
                    Row.Cells(EachC).Controls.Add(Censore)
                  End If
        ...
        If CurrentUser.Role(Setting.Name) >= Authentication.User.RoleType.Visitors Then
          ...
          If AddButtons Then
            ...
            If HttpContext.Current.Request.Browser.VBScript = True Then
              'TextToSpeek feature
              Dim IdButton As Integer = ValInt(CStr(HttpContext.Current.Items("LastIdContent"))) + 1
              HttpContext.Current.Items("LastIdContent") = IdButton
              Dim RssSource As String = ForumManager.Link(ForumManager.ActionType.ShowXML, Setting.Name, True, Nothing, ForumId, TopicID)
              MasterPage.AddButton(Phrase(Setting.Language, 3246), "vbscript:XmlSrc(""" & RssSource & """)", Nothing, IconType.MusicPlayer)
              Buttons.Controls.Add(Components.Button(Setting, Phrase(Setting.Language, 3246), "vbscript:XmlSrc(""" & RssSource & """)", Nothing, IconType.MusicPlayer))
            End If

            'Add button for add topic in google bookmarks
            Dim Bookmark As String = ForumManager.Link(ForumManager.ActionType.Show, Setting.Name, True, Nothing, ForumId, TopicID, 0, 1)
            Dim Href As String = HrefGoogleAddBookmarks(Setting.Language, Topic.Title, Bookmark, PathCurrentUrl)
            Buttons.Controls.Add(Components.Button(Setting, Phrase(Setting.Language, 134), Href, Nothing, Components.IconType.Favourites, "_blank", True))

            'Add reply button
            If OperationsColumn Then
              If Not Topic.Closed Then
                Dim AddButtonReply As Boolean = False
                If CurrentForum.Type <> Forum.TypeOfForum.TicketSystem Then
                  AddButtonReply = True
                Else
                ...
                If AddButtonReply Then
                  Dim Text As String
                  Dim HrefReply As String = ForumManager.Link(ForumManager.ActionType.Reply, Setting.Name, False, Jump, ForumId, TopicID)
                  ...
                  Else
                    ...
                    End Select
                  End If
                  MasterPage.AddButton(Text, HrefReply, Nothing, Components.IconType.Pen, MasterPageEnhanced.TargetForButton.Self, True, Nothing, True)
                  Dim ReplyButton As Control = Components.Button(Setting, Text, HrefReply, Nothing, Components.IconType.Pen, Nothing, True)
                  MasterPage.Suggest(ReplyButton)
            ...
            If CurrentUser.Role(Setting.Name) >= Authentication.User.RoleType.AdministratorJunior Then
              If Topic.Closed Then
                Href = Common.Href(Setting.Name, False, "messenger.aspx", MessengerManager.QueryStringParameters(, MessengerManager.Action.OpenTopic, False, Nothing, ForumId, Topic.ID))
                Buttons.Controls.Add(Components.Button(Setting, Phrase(Setting.Language, 2024), Href, Nothing, Components.IconType.Globe, Nothing, True))
              Else
                Href = Common.Href(Setting.Name, False, "messenger.aspx", MessengerManager.QueryStringParameters(, MessengerManager.Action.CloseTopic, False, Nothing, ForumId, Topic.ID))
                Buttons.Controls.Add(Components.Button(Setting, Phrase(Setting.Language, 111), Href, Nothing, Components.IconType.Globe, Nothing, True))
              End If
            End If
            ...
            If CurrentForum.Type = Forum.TypeOfForum.TicketSystem Then
              If Not Topic.Closed Then
                ...
                If CurrentUser.Role(Setting.Name) >= Authentication.User.RoleType.AdministratorJunior OrElse (CurrentUser.Username = FirstReply.Author AndAlso FirstReply.AuthorAccount Is Nothing) Then
                  If CBool(Topic.AsResolved) Then
                    Href = Common.Href(Setting.Name, False, "messenger.aspx", MessengerManager.QueryStringParameters(, MessengerManager.Action.MarkTopicAsResolved, False, "0", ForumId, Topic.ID))
                    Buttons.Controls.Add(Components.Button(Setting, Phrase(Setting.Language, 2027), Href, Nothing, Components.IconType.Globe, Nothing, True))
                  Else
                    Href = Common.Href(Setting.Name, False, "messenger.aspx", MessengerManager.QueryStringParameters(, MessengerManager.Action.MarkTopicAsResolved, False, "-1", ForumId, Topic.ID))
                    Buttons.Controls.Add(Components.Button(Setting, Phrase(Setting.Language, 2026), Href, Nothing, Components.IconType.Globe, Nothing, True))
                  End If
                End If
              End If
            End If
      ...
      If Not ForumForInsideUse(ForumId) AndAlso LinksCorrelated IsNot Nothing Then
        ...
        If Topic.SubCategory = 0 OrElse CurrentUser.Role(Setting.Name) >= Authentication.User.RoleType.AdministratorJunior Then
          If CurrentUser.Role(Setting.Name) >= Authentication.User.RoleType.Senior Then
            If Setting.Forums IsNot Nothing AndAlso Array.IndexOf(Setting.Forums, ForumId) <> -1 Then
              Dim Forum As Forum = CType(ForumManager.Forum.Load.GetItem(ForumId), ForumManager.Forum)
              If Forum.ForumStructure IsNot Nothing AndAlso Forum.ForumStructure.Categories.Count > 0 Then
                ...
                Dim Categories As Collections.Generic.List(Of ForumStructure.Category) = Forum.ForumStructure.CategoriesSorted
                For Each Category As ForumStructure.Category In Categories
                  For Each Subcategory As ForumStructure.Category.Subcategory In Category.Subcategories
                    ...
                    HyperLink.NavigateUrl = Link(ActionType.SetSubcategory, Setting.Name, False, Nothing, ForumId, TopicID, 0, 0, Nothing, Nothing, False, Nothing, Subcategory.Id)
                    Box.Controls.Add(BR)
                  ...
                  For Each Word As String In Words
                    For Each CompareWord As String In CompareWords
                      If CompareWord.Length >= 4 Then
                        Dim CompareWordSng = Left(CompareWord, Len(CompareWord) - 1) 'Cut the last caratter for not M/F case sensitive; Exemp.: "Hotels" > "Hotel"
                        If Math.Abs(Word.Length - CompareWordSng.Length) <= 3 Then
                          If CompareWordSng.StartsWith(Word, StringComparison.InvariantCultureIgnoreCase) OrElse Word.StartsWith(CompareWordSng, StringComparison.InvariantCultureIgnoreCase) Then
                            Finded += 1
                          Else
                            NotFinded += 1
                          End If
    ...
    Sub GetReplyText(AddToCell As HtmlTableCell, ForumId As Integer, TopicID As Integer, Reply As Reply, Setting As SubSite, Archive As Integer, NReply As Integer, Optional AddWWW As ContextualLink.AddWWW = ContextualLink.AddWWW.Auto)
      Dim Text As String = AddEmoticons(Reply.Text)
      If Reply.IsPoll Then
        PollManager.AddPoll(CurrentDomainConfiguration, Setting, Text, PollContext.Forum, Reply.Poll, ForumId, TopicID, Reply.ID)
      End If
      TagNameToLower(Text)

      If AddLinksAccommodationService IsNot Nothing Then
        AddLinksAccommodationService(Text, Setting.Language, CurrentDomainConfiguration, Setting.Name)
      End If

      ContextualLink.AddLinks(Text, Setting, CurrentDomainConfiguration, Archive, AddWWW)
      If NReply = 0 Then
        Text = AddCityInformation(Text, Setting)
      End If
      If Setting.SEO.CopyPrevention.ForumAndComments Then
        Text = ObfuscateHtml(Text, Setting)
      End If
    ...
    Private Sub SetRevisitAfrer(ByVal page As Page, ByVal LastReply As Reply)
      ...
      If page IsNot Nothing AndAlso LastReply IsNot Nothing Then
        Dim Master As Components.MasterPageEnhanced
        Master = CType(page.Master, Components.MasterPageEnhanced)
        ...
        Else
          ...
          If RevisitAfterDay < 1 Then
            RevisitAfterDay = 1
          End If
          Master.MetaRevisitAfterDays = RevisitAfterDay
        End If
      End If
    End Sub

    Function TitleTopicCtrl(ByVal ForumId As Integer, ByVal TopicId As Integer, ByVal Setting As SubSite, ByVal MasterPage As MasterPageEnhanced, Optional ByVal Reference As String = Nothing, Optional ByVal Text As String = Nothing) As Control
      Select Case ForumId
        Case ReservedForums.PhotoComment
        ...
        Case ReservedForums.ArchiveComment
          ...
          Dim Ctrl As New Control
          Components.AddPageArchived(Ctrl, MasterPage, CInt(APL(0)), CInt(APL(1)), HttpContext.Current, CurrentDomainConfiguration, Setting, Nothing, False, True, False)
          Return Ctrl
        Case Else
          ...
          Select Case ForumId
            ...
            Case Else
              ...
              Title.Text = AddCityInformation(Title.Text, Setting)
          End Select
          Return H1
      End Select
    End Function

    Public Sub AddButtonReturnForm(ByVal ForumId As Integer, ByVal Setting As SubSite, ByVal Masterpage As MasterPageEnhanced, ByVal Buttons As Control)
      Dim Text As String
      Dim Href As String
      Text = Phrase(Setting.Language, 61)
      Href = ForumManager.Link(ForumManager.ActionType.Show, Setting.Name, False, Nothing, ForumId)
      Masterpage.AddButton(Text, Href, Nothing, Icon_Forum)
      Buttons.Controls.Add(Components.Button(Setting, Text, Href, Nothing, Icon_Forum))
    End Sub

    Public Delegate Sub AddLinksDelegate(Text As String, Language As LanguageManager.Language, DomainConfiguration As DomainConfiguration, CurrentSubSiteName As String)
