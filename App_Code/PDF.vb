...
Namespace WebApplication
    ...
    Function PdfSupported(Language As LanguageManager.Language) As Boolean
    ...
    Function HrefPdf(ByVal Source As SourceType, ByVal DomainConfiguration As DomainConfiguration, ByVal SubSite As String, ByVal Base As Object, ByVal Id As Integer, ByVal FileName As String) As String
      ...
      NameFilePdf.Append(ReplaceBin(SubSite, "_", "?"))
      NameFilePdf.Append("_")
      ...
      Return Href(DomainConfiguration, Nothing, False, NameFilePdf.ToString)
    ...
    Public Sub BodyToPdf(ByRef clPDF As clsPDFCreator, ByVal Html As String)
      ...
      If Not Html Is Nothing Then
        ...
        For N As Integer = 0 To Html.Length - 1
          ...
          Select Case Chr
            Case "<"c
              HtmlControlToPdf(clPDF, Text, TagName, TextPrevCtrl, NList)
              Tag = True
              FlagTagName = False
              TagName = ""
            Case ">"c
          ...
          End Select
        Next
        HtmlControlToPdf(clPDF, Text, TagName, TextPrevCtrl, NList, True)

        'Return HttpUtility.HtmlDecode(StringBuilder.ToString)
      End If
    End Sub
    Private Sub HtmlControlToPdf(ByRef clPDF As clsPDFCreator, ByRef Text As String, ByVal TagName As String, ByRef TextBefore As String, ByRef NList As Integer, Optional ByVal IsLast As Boolean = False)
      Text = HttpUtility.HtmlDecode(Text)
      Dim FontSize As Single
      Dim PrintNow As Boolean = True
      Dim Style As PdfManager.PdfTextType = PdfTextType.DefaultText
      Select Case LCase(TagName)
        ...
        Case "h6"
          ...
          FontSize = 13
        Case "ul"
          'Parte una lista puntata
          NList = 0
        Case "ol"
          'Parte una lista numerata
          NList = 1
        Case "li"
          FontSize = 12
          If NList = 0 Then
    ...
    Public Function PdfDocument(Url As String) As Boolean
      ...
      If Url.EndsWith(".pdf") Then
        ...
        Dim Id As Integer
        Try
          Type = CType(CInt(Params(0)), PdfManager.SourceType)
          SubSiteName = ReplaceBin(Params(1), "?", "_")
          Base = ReplaceBin(CStr(Params(2)), "-", "/")
          Id = CInt(Params(3))
        ...
        PdfDocument(Type, SubSiteName, Base, Id)
        Return True
      End If
      Return False
    End Function

    Public Sub PdfDocument(Source As SourceType, SubSiteName As String, Base As Object, Id As Integer)
      Dim Setting As SubSite = SubSite.GetSubSite(SubSiteName)
      ...
      Select Case Source
        ...
        Case SourceType.Archive
          If Setting.EnablePdfVersion.Pages = EnabledStatus.No OrElse (Setting.EnablePdfVersion.Pages = EnabledStatus.Default AndAlso Setup.RenderingEngine.EnablePdfVersion.Pages = False) Then
            'Redirect
            Dim MenuArchive As MenuManager.Menu = MenuManager.Menu.Load(CInt(Base), Setting.Language)
            If MenuArchive IsNot Nothing Then
              Dim ItemMenuOfPage = MenuArchive.ItemsMenu.Page(Id)
              If ItemMenuOfPage IsNot Nothing Then
                HttpContext.Current.Response.Redirect(ItemMenuOfPage.Href(CurrentDomainConfiguration, Setting), True)
              End If
            End If
      ...
      With clPDF
        ...
        Dim PhotoInstance As PhotoManager.Photo = Nothing
        Select Case Source
          Case SourceType.Forum
            Dim Topic As Topic = ForumManager.Topic.Load(CInt(Base), Id)
            ...
            Else
              ...
              AddFooterToPdfDoc(clPDF, Host & ForumManager.Link(ForumManager.ActionType.Show, Setting.Name, False, Nothing, CInt(Base), Id, 0, 1))
              Dim Replies As Reply() = ForumManager.Topic.AllReplies(CInt(Base), Id, New UserPreferences(CurrentUser(Nothing)).ShowCensored, EnableShowHidden)

              'Load all Photos
              For Each Reply As Reply In Replies
                If Not String.IsNullOrEmpty(Reply.Photo) Then
                  PhotoInstance = PhotoManager.Photo.Load(Reply.Photo)
                  If PhotoInstance IsNot Nothing Then
                    clPDF.LoadImg(Alphanumeric(PhotoInstance.Album & "id" & PhotoInstance.Name), PhotoInstance.Image(Setting))
              ...
              For Each Reply As Reply In Replies
                ...
                If Not String.IsNullOrEmpty(Reply.Author) Then
                  Author = Reply.Author
                Else
                  Author = "?"
                End If
                AddTextToPdfDoc(clPDF, PdfTextType.Bold, Author & ":")
                If Reply.QuoteID <> 0 AndAlso Not String.IsNullOrEmpty(Reply.TextQuote) Then
                  Dim ReplyQuote As ForumManager.Reply = ForumManager.Reply.Load(CInt(Base), Id, Reply.QuoteID)
                ...
                If Not String.IsNullOrEmpty(Reply.Photo) Then
                  PhotoInstance = PhotoManager.Photo.Load(Reply.Photo)
                  If PhotoInstance IsNot Nothing Then
                    AddPhoto(clPDF, PhotoInstance, Setting)
                    PhotoInstance.Dispose()
                  End If
                End If
            ...
            End If
          Case SourceType.Photo
            PhotoInstance = PhotoManager.Photo.Load(Id, CStr(Base))
            If PhotoInstance Is Nothing Then
              HttpContext.Current.Response.RedirectPermanent(Href(Setting.Name, False, "default.aspx"), True)
            Else
              Dim AlbumLower As String = CStr(Base).ToLower()
              Dim ObjAlbum As PhotoAlbum = CType(PhotoManager.PhotoAlbum.Load.GetItem(CStr(Base)), PhotoAlbum)
              ...
              'Image = Photo.Image
              Inizialize(clPDF, Stream, PhotoInstance.Title(Setting.Language), PhotoInstance.Description(Setting.Language), PhotoInstance.Title(Setting.Language), PhotoInstance.Created)
          ...
          Case SourceType.Archive
            ...
            Dim Create As Date = TextToDate(MetaTags.MetaTag("date"))
            ...
            If Not String.IsNullOrEmpty(PhotoName) Then
              PhotoInstance = PhotoManager.Photo.Load(PhotoName)
              If PhotoInstance IsNot Nothing Then
                clPDF.LoadImg(Alphanumeric(PhotoInstance.Album & "id" & PhotoInstance.Name), PhotoInstance.Image(Setting))
              End If
            End If

            AddFooterToPdfDoc(clPDF, MenuManager.ItemMenu.Href(Setting, Id, CInt(Base), MetaTags.Title, True))
            .BeginPage()
    ...
    Private Sub AddPhoto(ByVal clPDF As clsPDFCreator, ByVal Photo As Photo, ByVal Setting As SubSite)
      ...
      If Not String.IsNullOrEmpty(Photo.Description(Setting.Language)) Then
    ...
    Private Sub AddTextToPdfDoc(ByVal clPDF As clsPDFCreator, ByVal TextType As PdfTextType, ByVal Text As String, Optional ByVal FontSize As Single = 12, Optional ByVal Color As Integer = &H0)
      ...
      For Each Line As String In Lines
        For Each SubLine As String In SplitTextBySize(clPDF, Line, Font, FontSize, MaxWidth)
          If Not String.IsNullOrEmpty(SubLine) Then
            Rows.Add(SubLine)
          End If
        Next
      Next
  ...
  Public Class clsPDFCreator
    ...
    Public Sub ClosePDFFile()
      ...
      WriteObj("<<" & vbCrLf & _
       IfStr(Not String.IsNullOrEmpty(strFont), "/Font <<" & vbCrLf & strFont & ">>" & vbCrLf, "") & _
       "/ProcSet [/PDF /Text" & IfStr(Not String.IsNullOrEmpty(strImg), " /ImageB /ImageC /ImageI", "") & " ]" & vbCrLf & _
       IfStr((Not String.IsNullOrEmpty(strImg)) Or (Not String.IsNullOrEmpty(strObject)), "/XObject <<" & vbCrLf & strImg & vbCrLf & strObject & ">>" & vbCrLf, "") & _
       ">>")
      EndObj()
    ...
    Private Function InitObj(Optional ByVal intObj As Integer = 0) As Integer
      ...
      arrXREF(intObj) = Extension.Right("0000000000" & CStr(Stream.Position - 1L), 10) & " 00000 n"
    ...
    Private Sub EndObj()
      Dim Bytes As Byte() = StringToByteArray("endobj" & vbCrLf, True)
      Stream.Write(Bytes, 0, Bytes.GetLength(0))
      'PrintLine(mvarFileNumber, "endobj")
    End Sub

    Private Sub WriteObj(ByVal strTemp As String)
      Dim Bytes As Byte() = StringToByteArray(strTemp & vbCrLf, True)
      Stream.Write(Bytes, 0, Bytes.GetLength(0))
    ...
    Public Sub SetTextRenderingMode(ByVal W As Single)
      If (W >= 0) AndAlso (W <= 2) Then WriteObj(ToStr(W) & " Tr")
    End Sub
    ...
    Public Sub SetDash(ByVal dash_on As Single, Optional ByVal dash_off As Single = 0)
      If (dash_on = 0) AndAlso (dash_off = 0) Then
        WriteObj("[ ] 0 d")
      Else
    ...
    Public Sub SetLineCap(ByVal W As Integer)
      If (W >= 0) AndAlso (W <= 2) Then WriteObj(ToStr(W) & " J")
    End Sub
    ...
    Optional ByVal Options As pdfPathOptions = pdfPathOptions.Stroked)
      ...
      For i = StartAngle To EndAngle Step Quality
        rad = C * i
        X2 = Ray * CSng(Math.Cos(rad))
        Y2 = (Ray * Ratio) * CSng(Math.Sin(rad))

        x1 = X2 * cTeta + Y2 * sTeta
        y1 = -X2 * sTeta + Y2 * cTeta
        If (i = StartAngle) AndAlso (Not Pie) Then MoveTo(x + x1, y + y1)
    ...
    Optional ByVal ColorSpace As pdfColorSpace = pdfColorSpace.pdfRGB)
      ...
      Select Case ImgBPP
        ...
        Case 8, 4, 1
          WriteObj("/BitsPerComponent " & CStr(ImgBPP) & vbCrLf & _
           ...
           CSng(Math.Pow(2, ImgBPP)) - 1 & " " & CStr(i + 2) & " 0 R]" & vbCrLf & _
           "/Length " & CStr(i + 1) & " 0 R")
      ...
      InitObj()
      WriteObj((sPixel.Length - IfInt(mvarEncodeASCII85, 2, 0)).ToString())
      EndObj()
      ...
      If ImgBPP <= 8 Then
        ...
        WriteObj("<<" & _
         ...
         "/Length " & (Len(sColor) - IfInt(mvarEncodeASCII85, 2, 0)) & " >>" & vbCrLf & _
         "stream" & vbCrLf & _
         sColor & vbCrLf & _
         "endstream")
        EndObj()
    ...
    Public Sub LoadFont(ByVal Name As String, ByVal BaseFont As String, Optional ByVal Options As System.Drawing.FontStyle = Drawing.FontStyle.Regular)
      ...
      With arrFONT(intUsedFont)
        ...
        EndObj()
        strFont = strFont & "/" & Name & " " & CStr(i) & " 0 R " & vbCrLf
        strFontname = strFontname & "<" & Name & ">" & Extension.Right("0000" & CStr(intUsedFont), 4) & ";"
    ...
    Private Function ToStr(ByVal Value As Single, Optional ByVal Dec As Integer = 3) As String
      Return CStr(Value.ToString("0.0000000000".Substring(0, 2 + Dec), Globalization.CultureInfo.InvariantCulture))
    End Function
    ...
    Public Function Length(ByVal Phrase As String, ByVal FontName As String, ByVal Fontsize As Single) As Single
      ...
      If k > 0 Then
        k = k + Len(FontName) + 2
        UsedFont = ValInt(Mid(strFontname, k, 4))

        With arrFONT(UsedFont)
          For i = 1 To l
            C = Asc(Mid(Phrase, i, 1))
            k = k + IfInt((C >= .FirstChar) AndAlso (C <= .LastChar), .Widths(C), .MissingWidth)
    ...
    Private Function ToASCII85(ByRef InBuf() As Byte) As String
      ...
      lngLen = UBound(InBuf)
      Dim UBarrASCII As Integer = CInt((lngLen * 1.3! + 1))
      Dim arrASCII(UBarrASCII) As Byte
      ...
      While k <= lngLen
        ...
        If (m = 0) AndAlso (l = 4) Then
          arrASCII(j) = CByte(Asc("z"))
          j = j + 1
        Else
    ...
    Private Function ReadBMP(ByVal Image As System.Drawing.Image, ByRef ImgBuf() As Byte, ByRef ImgColor() As Byte, ByRef ImgWidth As Integer, ByRef ImgHeight As Integer, ByRef ImgBPP As Byte, Optional ByVal ColorSpace As pdfColorSpace = pdfColorSpace.pdfRGB) As Boolean
      ...
      If ToBitplane <= 8 Then
        ...
        For Each EachColor As Drawing.Color In Colors
          'read palette color
          ImgColor(IdColor) = EachColor.R >> 1 'c#
          ImgColor(IdColor + 1) = EachColor.G >> 1 'c#
          ImgColor(IdColor + 2) = EachColor.B >> 1 'c#
          IdColor += 3
          Dim Argb As Integer = EachColor.ToArgb
          IndexPalette(Id) = Argb
          Id += 1
        Next
      ...
      Dim P As Integer = 1
      Dim EachPixel As Drawing.Color
      For y As Integer = Bitmap.Height - 1 To 0 Step -1
        For x As Integer = 0 To Bitmap.Width - 1
          If ToBitplane <= 8 Then
            ...
            P += 1
          Else
            EachPixel = Bitmap.GetPixel(x, y)
            ImgBuf(P) = EachPixel.R >> 1 'c#
            ImgBuf(P + 1) = EachPixel.G >> 1 'c#
            ImgBuf(P + 2) = EachPixel.B >> 1 'c#
            P += 3
          End If
        Next
      Next
    ...
    Private Function ToPdfStr(ByVal Temp As String) As String
      If Not String.IsNullOrEmpty(Temp) Then
        ...
        For Each C As Char In Temp
          ...
          If Not (Ascii >= 32 AndAlso Ascii <= 127) Then
    ...
    Private Function CreateFont(ByVal FontName As String, ByVal Style As System.Drawing.FontStyle) As FontDescriptor
      ...
      Dim StemH As Integer = 0, ItalicAngle As Integer = 0
      Dim Flag As Integer = 0
      Dim MaxWidth As Integer = 0, Height As Integer = 0, bx3 As Integer = 0, bx4 As Integer = 0

      For N As Integer = 32 To 255
        Dim Id As Integer = N - 31
        Size = gr.MeasureString(CStr(Chr(N)), Font, 0)
      ...
      Next
      gr.Dispose()
      awTemp(1) = awTemp(2) 'Set space Length
      ...
      If CBool(Style And Drawing.FontStyle.Strikeout) Then
        AddToBase &= "Strikeout"
      End If
      If Not String.IsNullOrEmpty(AddToBase) Then
      ...
      If awTemp(1) = awTemp(6) Then
        Flag = Flag Or 1
        bx3 = CInt(CSng(bx4) * 0.8!)
      Else
        bx3 = CInt(CSng(bx4) * 1.2!)
      End If

      fdTemp.Param = "/Flags " & Flag & " /FontBBox [-250 -300 " & bx3 & " " & bx4 & "] " & _
      "/MissingWidth " & fdTemp.MissingWidth & " /StemV 109 " & _
      "/StemH " & StemH & " /ItalicAngle " & ItalicAngle & "  /CapHeight " & Height & " /XHeight " & CInt(Height / 2) & " " & _
    ...
    Private Sub InsertObjectOnPage()
      ...
      If intObject > 0 Then
        For i = 1 To intObject
          Dim Options As Integer = CInt(arrOBJECT(i).Options)
          'C# version (DONT REMOVE!): if (System.Convert.ToBoolean(Options & 7) || (System.Convert.ToBoolean(Options & 2) && !System.Convert.ToBoolean(mvarPages % 2)) || (System.Convert.ToBoolean(Options & 4) && !System.Convert.ToBoolean(mvarPages % 2)) && (!(System.Convert.ToBoolean(Options & 8) && (mvarPages == 1))))
          If CBool(Options And 7) OrElse (CBool(Options And 2) AndAlso Not CBool(mvarPages Mod 2)) OrElse (CBool(Options And 4) AndAlso Not CBool(mvarPages Mod 2)) AndAlso (Not (CBool(Options And 8) AndAlso (mvarPages = 1))) Then
            DrawObject(arrOBJECT(i).Name)
          End If
        Next
      End If
    End Sub
