...
Partial Class ConfigEditor
  ...
  Shared Plugin As New PluginManager.Plugin(, , , , PluginManager.Plugin.Characteristics.CorePlugin)
	Shared Progressive As Integer
  Private EditObjectParameters As Common.EditObjectParameters
    ...
    CurrentUser = Authentication.CurrentUser(Session)
		...
		ElseIf Request.QueryString("edit") IsNot Nothing Then
            ...
            '                                 Config.SaveConfiguration("config.xml")
            '                               End Sub
            Dim Save As SaveObjectMethod = AddressOf SaveSetup
            EditObjectParameters.ObjectToEdit = Config.Setup
      ...
      EditObjectParameters = CType(RetriveObjectFromOtherPage(), Common.EditObjectParameters)
			Progressive += 1
			KeyObject.Value = "KeepAlive" & Progressive
			Session(KeyObject.Value) = EditObjectParameters
		Else
      EditObjectParameters = CType(Session(Request.Form(KeyObject.UniqueID)), Common.EditObjectParameters)
		End If
  End Sub
  ...
  End Sub
    ...
    Dim MasterPage As Components.MasterPageEnhanced = SetMasterPage(Me, Nothing, False, False)
      ...
      Dim SaveObjectMethod As Common.SaveObjectMethod = EditObjectParameters.SaveObjectMethod
      ...
      Dim TablePropertyIdPhraseCorrispondence As Collections.Generic.Dictionary(Of String, Integer) = EditObjectParameters.TablePropertyIdPhraseCorrispondence
        ...
        Dim RetriveSetError As Boolean = False
        ...
        Dim BackError As Boolean = False 'Variable istanced for c# campatibility
        PlaceHolder1.Controls.Add(ObjectToControl(Setting, Obj, TablePropertyPhraseCorrispondence, TablePropertyIdPhraseCorrispondence, NamePhraseBooks, Nothing, BackError, EditObjectParameters.FilterExcludeProperties, EditObjectParameters.FilterIncludeProperties, EditObjectParameters.FilterArray))
			End If
		Else
