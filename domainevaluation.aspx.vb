...
Partial Class domainevaluation
  ...
  End Enum
    ...
    If Plugin Is Nothing Then Plugin = New PluginManager.Plugin(AddressOf Description, Authentication.User.RoleType.Visitors, True, True)
		Return Plugin
	End Function
	Shared Sub New()
		Initialize()
  ...
  Protected Shared Sub Plugin_MasterPagePreRender(ByVal MasterPage As Components.MasterPageEnhanced) Handles Plugin.MasterPagePreRender
    If MasterPage.ShowTopBar Then
      ...
      If Plugin.IsEnabledAndAccessible(MasterPage.User, Setting) Then
        ...
        MasterPage.AddButton(Phrase(Setting.Language, 5002), Common.Href(Setting.Name, False, "domainevaluation.aspx"), Nothing, Components.IconType.Globe)
      End If
    End If
  End Sub
  ...
  Protected Sub domainevaluation_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    Action = CType(ValInt(Request.QueryString("a")), ActionType)
  ...
  Sub ResponseCertificate()
    ...
    Dim FontSizeDomine As Integer = CInt(34 / DomainName.Length * 15)
    ...
    Dim Font3 As New Drawing.Font(FontName, CSng(FontSize) / 1.5!, Drawing.FontStyle.Regular, Drawing.GraphicsUnit.Pixel)
    ...
    Dim Position As New Drawing.PointF(CSng(Image.Width / 2), 46)
    g.DrawString("Certificate of attributed value", Font, Brush, Position, Format)

    Position = New Drawing.PointF(CSng(Image.Width / 2), 82)
    g.DrawString(DomainName, Font2, Brush2, Position, Format)

    Position = New Drawing.PointF(CSng(Image.Width / 2), 123)
    g.DrawString("This web domain has been estimated:", Font, Brush, Position, Format)

    Position = New Drawing.PointF(CSng(Image.Width / 2), 123 + FontSize + 4)
  ...
  Function CheckId(ByVal DomainName As String) As String
    ...
    For N As Integer = 0 To Domain.GetUpperBound(0) - 6
      Dim Value As Long = CLng(Microsoft.VisualBasic.Val("&H" & Hex2(Domain(N)) & Hex2(Domain(N + 1)) & Hex2(Domain(N + 2)) & Hex2(Domain(N + 3)) & Hex2(Domain(N + 4)) & Hex2(Domain(N + 5))))
  ...
  Function CheckDomainName(ByRef Domain As String) As Boolean
    ...
    If Domain IsNot Nothing Then
      ...
      If CBool(Position) Then
        Domain = Extension.Left(Domain, Position - 1)
      End If
  ...
  Private Function Hex2(ByVal C As Char) As String
    Return Extension.Right("00" & Hex(Asc(C)), 2)
  End Function
  ...
  Protected Sub domainevaluation_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Dim MasterPage As Components.MasterPageEnhanced = SetMasterPage(Me, Nothing, True, True)
    'Add CSS
    ...
    If CallFromHomePage Then
      If Not Page.IsPostBack Then
        If Setting.Aspect.FirstDocumentInHomePage Then
          ...
          Dim ArchiveNumber As Integer = 0
  ...
  Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
    If DateDiff(DateInterval.Second, CDate(Session("TimePageLoaded")), Now) < 4 Then
      'Block auto robot sublit
      Panel1.Controls.Add(Components.Message(Phrase(Setting.Language, 422, 3245), Setting, Nothing, Nothing, MessageType.ErrorAlert))
      Panel1.Visible = True
    Else
      Dim DomainName As String = Domain.Text 'c# compatibility
      If CheckDomainName(DomainName) Then
        ...
        Else
          'Application error
          Panel1.Controls.Add(Components.Message(Setting, 124, 0, Nothing, MessageType.ErrorAlert))
          Panel1.Visible = True
        End If
      Else
        'Value not valid (domain name not valid)
        Domain.Text = DomainName
        Panel1.Controls.Add(Components.Message(Setting, 400, 0, Nothing, MessageType.ErrorAlert))
        Panel1.Visible = True
      End If
    End If
  End Sub
  ...
  Function EvaluationDomain(ByVal Domain As String) As Long
    If CheckDomainName(Domain) Then
      If EvaluationsEvaluation.Contains(Domain) Then
        Return CLng(EvaluationsEvaluation(Domain))
      Else
        Try
          Dim EvaluationSiteOld As Long = 0L, EvaluationTrafficRanks As Long = 0L, EvaluationDomainName As Long = 0L
          ...
          If EvaluationDomainName > 1000 AndAlso Days < 365 Then
  ...
  Function EvaluationSiteOld(ByVal Days As Integer) As Long
    Return CLng((Days / 130) ^ 2)
  End Function

  Function DaysSiteOldFromWhois(ByVal Domain As String) As Integer
    Dim WhoisResult As String = ReadHtmlFromWeb("http://www.who.is/whois/" & Domain, System.Text.Encoding.GetEncoding("windows-1251"))
    ...
    Dim N As Integer = 0, LenDate As Integer = 0
    For Each Find As String In Finds
      ...
      If CBool(Position) Then
        ...
        End Select
        Return CInt(DateDiff(DateInterval.Day, Created, Today))
      End If
      N += 1
    Next
    Return 0
  ...
  Function EvaluationDomainName(ByVal Domain As String) As Long
    Dim FileExtension As String = Mid(Domain, InStr(Domain, ".") + 1)
    Dim DomainName As String = WebApplication.Extension.Left(Domain, InStr(Domain, ".") - 1)
    Dim Meta As String = Nothing
    If Len(FileExtension) = 2 Then
      Meta = "&meta=lr%3Dlang_" & FileExtension
    End If
    ...
    If CBool(DidYouMeanPosition) Then
  ...
  Function EvaluationTrafficRanks(ByVal Domain As String, Optional ByRef EvaluationSiteOld As Long = 0, Optional ByRef Days As Integer = 0) As Long
    Dim Html As String = Nothing, TraficRank As Long
    TraficRank = CLng(AlexaTrafficRank(Domain, Html))
    ...
    If TraficRank <> 0 Then
      Dim Value As Long = CLng((500000000000 / (TraficRank + 4) ^ 1.3) - 300)
    ...
    If CBool(Position) Then
      ...
      Try
        If Since.EndsWith("</") Then
          Since = Extension.Left(Since, Since.Length - 2)
