...
Partial Class translator
  ...
  Protected Shared Sub Plugin_MasterPagePreRender(ByVal MasterPage As Components.MasterPageEnhanced) Handles Plugin.MasterPagePreRender
    If MasterPage.ShowTopBar Then
      ...
      If Plugin.IsEnabledAndAccessible(MasterPage.User, Setting) Then
        MasterPage.AddButton(Phrase(Setting.Language, 3222), Common.Href(Setting.Name, False, "translator.aspx"), Nothing, Components.IconType.Globe)
      End If
    End If
  End Sub
  ...
  Protected Sub translator_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    If Page.IsPostBack Then
      SelectedSource = CType([Enum].Parse(GetType(LanguageManager.Language), Request.Form(SourceLanguage.UniqueID)), LanguageManager.Language)
      SelectedTarget = CType([Enum].Parse(GetType(LanguageManager.Language), Request.Form(TargetLanguage.UniqueID)), LanguageManager.Language)
    Else
      Dim Value As String = Cookie("LastTranslation")
      If Not String.IsNullOrEmpty(Value) Then
        Dim Values As String() = Split(Value)
        SelectedSource = CType(Int32.Parse(Values(0)), LanguageManager.Language)
        SelectedTarget = CType(Int32.Parse(Values(1)), LanguageManager.Language)
      Else
  ...
  Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
    ...
    If DateDiff(DateInterval.Second, PageLoaded, Now) < 10 Then
      Dim MasterPage As Components.MasterPageEnhanced = CType(Page.Master, Components.MasterPageEnhanced) : MasterPage.AddMessage(Phrase(Setting.Language, 422, 3245), Setting)
    Else
      Dim Source As LanguageManager.Language = CType(Int32.Parse(Request(SourceLanguage.UniqueID)), LanguageManager.Language)
      Dim Target As LanguageManager.Language = CType(Int32.Parse(Request(TargetLanguage.UniqueID)), LanguageManager.Language)
      ...
      Else
        ...
        Catch ex As Exception
          Dim MasterPage As Components.MasterPageEnhanced = CType(Page.Master, Components.MasterPageEnhanced) : MasterPage.AddMessage(Setting, 3226, 0, Nothing, MessageType.ErrorAlert)
        End Try
      End If
  ...
  End Sub
	...
	Sub PupulateLanguageList(ByVal List As WebControls.DropDownList, ByVal Selected As LanguageManager.Language)
        ...
        Case LanguageManager.Language.Chinese, LanguageManager.Language.Dutch, LanguageManager.Language.English, LanguageManager.Language.French, LanguageManager.Language.Italian, LanguageManager.Language.Japanese, LanguageManager.Language.Russian, LanguageManager.Language.Spanish
          Item = New WebControls.ListItem
          Item.Value = CStr(Language)
