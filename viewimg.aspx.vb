...
#End Region
  ...
  Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    If Not String.IsNullOrEmpty(Request.QueryString(QueryKey.ImageName)) Then
      Try
        Name = CInt(Request.QueryString(QueryKey.ImageName))
      ...
      Title = Photo.Title(CType(Int32.Parse(Request.QueryString(QueryKey.LanguageImage)), LanguageManager.Language))
      Description = Photo.Description(CType(Int32.Parse(Request.QueryString(QueryKey.LanguageImage)), LanguageManager.Language))
      Dim PhotoAlbum As PhotoAlbum = CType(PhotoManager.PhotoAlbum.Load.GetItem(Photo.Album), PhotoManager.PhotoAlbum)
      ...
      End If
      Subject = CType(PhotoManager.PhotoAlbum.Load.GetItem(Photo.Album), PhotoManager.PhotoAlbum).Title(CType(Int32.Parse(Request.QueryString(QueryKey.LanguageImage)), LanguageManager.Language))
    Else
    ...
    Try
      Size = CInt(Request.QueryString(QueryKey.Size))
    Catch ex As Exception
      Error404()
    End Try
    FullSize = Not String.IsNullOrEmpty(Request.QueryString(QueryKey.Fullsize))
  ...
  Sub SendFile(ByRef File As String, ByVal Title As String, ByVal Description As String, ByVal Subject As String)
    ...
    If Album Is Nothing OrElse Not Album.StartsWith("users/") OrElse Setting.Aspect.ImageRendering.DoNotApplyTheImagesToneToUsers = False Then
      Dim ColorTone As ColorTone = ImageManager.ColorTone.Default
      Dim Tone As String = Request.QueryString(QueryKey.ImageColorTone)
      If Tone IsNot Nothing Then
        ColorTone = CType(Int32.Parse(Tone), ImageManager.ColorTone)
      End If
      Image = ApplyColorTone(Image, ColorTone, Setting)
    End If

    Dim Obfuscation As Boolean = False
    ...
    If Not FullSize OrElse Obfuscation Then
      ...
      If Obfuscation Then
        ...
        If Size = 0 Then
          Size = Config.ShowDefaultImageSize(HttpContext.Current)
        ...
        If Image.Width > Image.Height Then
          Width = Size
        Else
          Width = CInt(Size * Image.Width / Image.Height)
        End If
        If Image.Height > Image.Width Then
          Height = Size
        Else
          Height = CInt(Size * Image.Height / Image.Width)
        End If
    ...
    Try
      If Config.Setup.RenderingEngine.ImageCopyright AndAlso Size >= 150 Then
        'Add Copyright
        ...
        Try
          If Config.Setup.RenderingEngine.FontCopyright IsNot Nothing Then
            FontName = Config.Setup.RenderingEngine.FontCopyright.Name
            If Not Request.Browser.IsMobileDevice Then
              FontSize = Config.Setup.RenderingEngine.FontCopyright.Size
            End If
            Alpha = CInt((100.0! - CSng(Config.Setup.RenderingEngine.FontCopyright.TransparencyPercentage)) * 2.55!)
          End If
        Catch ex As Exception
    ...
    If Obfuscation Then
      Image = ImageManager.ImageRotate(CType(Image, Bitmap), 10)
    End If

    ' Set the contenttype
    ' send image to the viewer
    Dim ImageFormat As Drawing.Imaging.ImageFormat = Nothing
    Dim Parameters As Imaging.EncoderParameters = Nothing
    ...
    If Image.Flags = 2 OrElse Image.Palette.Entries.Length = 0 Then
      ...
      If Config.Setup.RenderingEngine.InterlaceJpegImages = True Then
        ...
        Parameters.Param(0) = New EncoderParameter(System.Drawing.Imaging.Encoder.ScanMethod, CInt(EncoderValue.ScanMethodInterlaced)) 'Cint necessary fot c# conversion
        Parameters.Param(1) = New EncoderParameter(System.Drawing.Imaging.Encoder.RenderMethod, CInt(EncoderValue.RenderProgressive)) 'Cint necessary fot c# conversion
      Else
  ...
  Sub SetPropertyItem(ByRef Image As System.Drawing.Image, ByVal Title As String, ByVal Description As String, ByVal Subject As String)
    If Config.Setup.SEO.AddDescriptiveExifInfoToImages Then
      ...
      If Not String.IsNullOrEmpty(Title) OrElse Not String.IsNullOrEmpty(Description) Then
        'Sec Keywords
        Dim Key As String = Title
        If Not String.IsNullOrEmpty(Title) AndAlso Not String.IsNullOrEmpty(Description) Then
          Key &= ","
        End If
        Key &= Description
