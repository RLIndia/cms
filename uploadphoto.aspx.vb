...
Partial Class uploadphoto
  ...
  Protected Sub Submit1_ServerClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles Submit1.ServerClick
    ...
    Try
      ...
      ElseIf Not String.IsNullOrEmpty(SourceUrl.Text) Then
      ...
      End If
      If Not String.IsNullOrEmpty(TitlePhoto.Value) Then
        Photo.Title(Setting.Language) = TitlePhoto.Value
      End If
      ...
      Response.Redirect(HrefPhoto(Photo.Album, Photo.Name, Setting), True)
  ...
  Protected Sub uploadphoto_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Dim Album As String
    If Not String.IsNullOrEmpty(Request.QueryString("album")) Then
    ...
    Submit1.Attributes.Add("onClick", "if(" & Page.Form.UniqueID & "." & InputFile.UniqueID & ".value==''&&" & Page.Form.UniqueID & "." & SourceUrl.UniqueID & ".value==''){" & Page.Form.UniqueID & ".setAttribute('target', '_self')}")
