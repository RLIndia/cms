  ...
  Private ToUser As User
	...
	Private Sub messenger_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
      ...
      If Not Request(QueryKey.MsgAction) Is Nothing Then
        Action = CType(ValInt(Request(QueryKey.MsgAction)), MessengerManager.Action)
      End If
      ...
      If Not Request(QueryKey.MsgForumId) Is Nothing Then
        ForumId = CInt(Request(QueryKey.MsgForumId))
      End If
      If Not Request(QueryKey.MsgTopicId) Is Nothing Then
        TopicID = CInt(Request(QueryKey.MsgTopicId))
      End If
      If Not Request(QueryKey.MsgReplyId) Is Nothing Then
        ReplyId = CInt(Request(QueryKey.MsgReplyId))
      End If
    ...
    CurrentUser = Authentication.CurrentUser(Session)
      ...
      Dim MasterPage As Components.MasterPageEnhanced = SetMasterPage(Me, Nothing, False, False, True, False)
          ...
          CurrentUser.SocialConfiguration.RemoveUserContact(ToUser.Username)
				...
				Case MessengerManager.Action.SendMessengerComunication
					...
					If Not Comunication Is Nothing Then
						...
						'url
            ListMessage.Item.Add(HrefPrivateChatRoom(Setting, Comunication.FromUser, Nothing, True))
						'target
      ...
      Case Else
          ...
          If Confirm Then
            ...
            Select Case Action
              ...
              Case MessengerManager.Action.MarkTopicAsResolved
                ForumManager.RequireSetTopicAsResolved(Setting, ForumId, TopicID, CInt(Value))
            End Select
            SetNextMessage(418)
          End If
          ReturnBack()
        Else
          Select Case Action
            ...
            Case MessengerManager.Action.RequireTopicCensore
              Dim Topic As ForumManager.Topic = ForumManager.Topic.Load(ForumId, TopicID)
              ...
              If Topic IsNot Nothing Then
                ...
                Else
                  ...
                  AddTxt = Extension.Left(Reply.InnerTextReply, 15) & "..."
                End If
              End If
	...
	Sub ReturnBack()
      ...
      Dim Back As String = CStr(Session("BackPage"))
