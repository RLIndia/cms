...
Partial Class CheckUp
  ...
  Shared Function Initialize() As PluginManager.Plugin
    If Plugin Is Nothing Then Plugin = New PluginManager.Plugin(AddressOf Description, Authentication.User.RoleType.Supervisor, True, False, PluginManager.Plugin.Characteristics.CorePlugin)
    Return Plugin
  End Function
  Shared Sub New()
    Initialize()
  ...
  Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    If CBool(AntyDDoSAttacks.IpDDoSAttacks.Count) Then
      Panel1.Visible = True
    End If
    ...
    While IdIp < AntyDDoSAttacks.IpDDoSAttacks.Count
      ...
      DDoSIp.Controls.Add(Components.Literal(" "))
      IdIp += 1
    End While

    UserStats.Controls.Add(Components.OnlineUser(Setting, False, True))
    ...
    For Each Server As String In Servers()
      If Not String.IsNullOrEmpty(MultiServer.Text) Then
        MultiServer.Text &= ","
      End If
    ...
    Try
      ...
      MB = CLng(Disk.TotalFreeSpace / 1024 ^ 2)
    ...
    TimeServer.Text = CStr(Now)
    TimeUTC.Text = CStr(Date.UtcNow)
    ...
    Try
      ...
      Dim ScoreResult As Long = CLng(Record - Integer.MinValue)
      Benchmark.Text = (ScoreResult / 134).ToString("#")
    Catch ex As Exception
    End Try
    ...
    Select Case ErrorLog.ErrorDiskSpacePreserved
      ...
      Case Is > 0
        Me.ErrorDiskSpacePreserved.Text = CStr(ErrorLog.ErrorDiskSpacePreserved)
    ...
    If Domains IsNot Nothing Then
      For Each Domain As String In Domains
        Dim DomainSetting As DomainConfiguration = Config.DomainConfiguration.Load(Domain)
        If String.IsNullOrEmpty(DomainSetting.Redirect) Then
          If Not DomainsNoConnect.Contains(Domain) Then
            Dim Url As String = Href(Domain, True, "default.aspx", QueryKey.Show, DefaultPageShowType.Ip)
            Dim Ok As Boolean
            Try
              ReadHtmlFromWeb(Url, Nothing, 15000)
              Ok = True
            Catch ex As Exception
              Ok = False
            End Try
