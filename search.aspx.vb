...
Partial Class search
  ...
  Private CurrentUser As User
    ...
    NPage = ValInt(Request.QueryString("np"))
    FindType = CType(ValInt(Request.QueryString("cr")), CriterionSearchType)
    ...
    Dim newImage As System.Drawing.Bitmap = Nothing
        ...
        Dim Image As System.Drawing.Image = Nothing
          ...
          End If
            ...
            If C1.ToArgb = -5434345 AndAlso C2.ToArgb = -1 AndAlso C3.ToArgb = -328966 Then
              NotFound = True
            Else
              NotFound = False
            End If
        ...
        Loop Until (NotFound = False AndAlso LoadError = False) OrElse Tr = 5


        If LoadError OrElse NotFound Then
    ...
    Dim MasterPage As Components.MasterPageEnhanced = SetMasterPage(Me, , True, True)
		'Add CSS
    ...
    For N As CriterionSearchType = CriterionSearchType.AllWords To CriterionSearchType.OneWords
  ...
  End Function
    ...
    Dim NRow As Integer = 1 : Dim Language As LanguageManager.Language = CurrentSetting().Language
		Select Case Type
			Case SearchType.Web
				If Preview Then
					NRow += 1
        ...
        Dim Query As String = Nothing
				Select Case FindType
					...
					Case CriterionSearchType.Phrase
						Query = """" & Q & """"
				End Select
        If Not String.IsNullOrEmpty(NotQ) Then
          Query &= " -(" & NotQ & ")"
        End If
        Request.XmlHref = "http://search.live.com/results.aspx?q=" & Query & "&format=rss&first=" & CStr(NPage * 10 + 1)
        TitleTable = Phrase(Language, 4005)
        ...
        TitleTable = Phrase(Language, 4006)
			Case SearchType.Video
        Request.XmlHref = "http://video.google.com/videofeed?type=search&so=3&num=20&output=rss&q=""" & Q & """" & "&hl=" & Acronym(Setting.Language) & "&start=" & CStr(NPage * 20)
        TitleTable = Phrase(Language, 3258)
		...
		End Select
    TitleTable = Phrase(Language, 4001) & " """ & TitleTable & """"
      ...
      Dim Words As String() = Extension.ExtrapolateWords(Q)
			...
			If NRow >= 2 Then
				Select Case Type
					Case SearchType.Web
						...
						'If Pslesh Then
            'Domain = Extension.Left(Domain, Pslesh - 1)
						'End If
  ...
  End Enum
    ...
    If Not (Npage = 0 AndAlso NotFound = True) Then
      If CBool(Npage) Then
  ...
  Sub SaveSearchSetting()
    ...
    End If
    Cookie("src_tb") = CStr(TargetBlank.Checked)
    Cookie("src_pr") = CStr(ShowPreview.Checked)
    Cookie("src_ws") = CStr(WebSearch.Checked)
    Cookie("src_ss") = CStr(SaleSearch.Checked)
    Cookie("src_vs") = CStr(VideoSearch.Checked)
    Cookie("src_qu") = CStr(QuestionSearch.Checked)
  End Sub
  ...
  Sub LoadSearchSetting()
    ...
    If CurrentUser.GeneralRole > Authentication.User.RoleType.Visitors Then
        ...
        Blank = CBool(CurrentUser.Attribute("src_tb"))
				SetFromUser = True
			End If
			If CurrentUser.Attribute("src_pr") IsNot Nothing Then
        Preview = CBool(CurrentUser.Attribute("src_pr"))
			End If
			If CurrentUser.Attribute("src_ws") IsNot Nothing Then
        SearchWeb = CBool(CurrentUser.Attribute("src_ws"))
			End If
			If CurrentUser.Attribute("src_ss") IsNot Nothing Then
        SearchSale = CBool(CurrentUser.Attribute("src_ss"))
			End If
			If CurrentUser.Attribute("src_vs") IsNot Nothing Then
        SearchVideo = CBool(CurrentUser.Attribute("src_vs"))
			End If
			If CurrentUser.Attribute("src_qu") IsNot Nothing Then
        SearchQuestion = CBool(CurrentUser.Attribute("src_qu"))
			End If
    End If
    If Not SetFromUser Then
      Try
        Blank = CBool(Cookie("src_tb"))
      Catch ex As Exception
        Blank = False
      End Try
      Try
        Preview = CBool(Cookie("src_pr"))
      Catch ex As Exception
        Preview = False
      End Try
      Try
        SearchWeb = CBool(Cookie("src_ws"))
      Catch ex As Exception
        SearchWeb = True
      End Try
      Try
        SearchSale = CBool(Cookie("src_ss"))
      Catch ex As Exception
        SearchSale = False
      End Try
      Try
        SearchVideo = CBool(Cookie("src_vs"))
      Catch ex As Exception
        SearchVideo = True
      End Try
      Try
        SearchQuestion = CBool(Cookie("src_qu"))
      Catch ex As Exception
  ...
  Protected Sub search_PreRenderComplete(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRenderComplete
    ...
    If String.IsNullOrEmpty(Query.Text) Then
      ...
      ScriptFocus.Controls.Add(Components.Script("document.all." & Query.ClientID & ".focus();", ScriptLanguage.javascript))
  ...
  Protected Sub StartSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles StartSearch.Click
    ...
    If Setup.Visible Then
      SaveSearchSetting()
    End If
    Response.Redirect(RequestUrl(ActionType.Search, , Query.Text, , CType(Page.Request.Form(FindCriterion.UniqueID), CriterionSearchType), NotTerms.Text))
  End Sub
End Class
