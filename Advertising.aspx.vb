...
Partial Class Advertising
  ...
  Shared Function Initialize() As PluginManager.Plugin
    If Plugin Is Nothing Then Plugin = New PluginManager.Plugin(AddressOf Description, Authentication.User.RoleType.Visitors, False, False, PluginManager.Plugin.Characteristics.StandardPlugin)
    Return Plugin
  End Function
  ...
  Protected Shared Sub Plugin_Ipn(ByVal IdOrder As String, ByVal Request As System.Web.HttpRequest) Handles Plugin.Ipn
    ActivationTextualLink(CInt(IdOrder))
  End Sub
  ...
  Protected Shared Sub Plugin_MasterPagePreRender(ByVal MasterPage As Components.MasterPageEnhanced) Handles Plugin.MasterPagePreRender
    ...
    If MasterPage.ShowRightBar Then
      ...
      If CBool(Links.Controls.Count) Then
  ...
  Sub Confirmation()
    ...
    Dim NDays As Integer = ValInt(Days.Text)
    ...
    For Each DomainName As String In AllDomainNames()
      ...
      If String.IsNullOrEmpty(Domain.Redirect) Then
        If LCase(Page.Request.Form(DomainName)) = "on" Then
          ...
          If Not String.IsNullOrEmpty(TextDomains) Then
            TextDomains &= " "
          End If
    ...
    If TotalPrice.Value = CStr(Total) Then
      ...
      TextualLink.Save()
      TextualLink.ID = CInt(SaveObject(TextualLink, Nothing, True))
      ...
      SendEmail(Subject, ControlToText(ActivationLink), Email.Text, True, True)
      Response.Redirect(PayUrl)
    Else
      MasterPage.AddMessage(Setting, 422, 0, Common.Href(MasterPage.Setting.Name, False, "contact.aspx"), MessageType.ErrorAlert)
    End If
  End Sub

  Sub OrderDetail()
    If Not String.IsNullOrEmpty(TotalPrice.Value) Then
      Confirmation()
    End If
  ...
  End Sub
	...
	Sub InputOrder()
      ...
      Components.AddPageArchived(Div, MasterPage, 0, 7, HttpContext.Current, CurrentDomainConfiguration, Setting)
    ...
    AddLabel(CType(TitleRow.Controls(0), HtmlTableCell), Phrase(Setting.Language, 56))  'Add
    AddLabel(CType(TitleRow.Controls(1), HtmlTableCell), Phrase(Setting.Language, 2011))  'DailiRate
    AddLabel(CType(TitleRow.Controls(2), HtmlTableCell), "PageRank")
    AddLabel(CType(TitleRow.Controls(3), HtmlTableCell), Phrase(Setting.Language, 125)) 'Domain name '
    AddLabel(CType(TitleRow.Controls(4), HtmlTableCell), Phrase(Setting.Language, 3026))  'Web site
		For Each DomainName As String In AllDomainNames()
      Dim Domain As Config.DomainConfiguration = Config.DomainConfiguration.Load(DomainName)
			Dim Enabled As Boolean
			Enabled = False
      For Each SubSite As String In Domain.SubSitesAvailable()
        If Plugin.IsEnabled(CType(Config.SubSite.Load.GetItem(SubSite), Config.SubSite)) Then
          Enabled = True
          Exit For
        End If
      Next
      If String.IsNullOrEmpty(Domain.Redirect) AndAlso Enabled Then
  ...
  End Function
    ...
    If Page.IsPostBack Then
      ...
      If Not IsSelected() Then
        MasterPage.AddMessage(Setting, 3001, 0, Nothing, MessageType.ErrorAlert)
        IsErr = True
      End If
      If Not IsValid Then
        MasterPage.AddMessage(Setting, 404, 0, Nothing, MessageType.ErrorAlert)
        IsErr = True
      End If
      If IsErr Then
        InputOrder()
...
Module AdvertisingManager
    ...
    If PluginManager.IsEnabled(Setting, "advertising.aspx") AndAlso PayPalAccountIsSetting() AndAlso Config.Setup.PaymentGateway.EnabledOnlinePayment Then
      Return True
    End If
    Return False
  End Function
    ...
    Public Created As Date = Now.ToUniversalTime()
      ...
      Return CType(Persistent.LoadObject(GetType(TextualLink), CStr(ID)), TextualLink)
    End Function
    Sub Save()
      SaveObject(Me, CStr(Me.ID), True)
    End Sub
  End Class
  ...
  Function DailyLinkValue(ByVal DomainName As String) As Double
    ...
    SyncLock SyncLockObj
      Dim TraficRank As Long, ValueLink As Double = 0.0
      If Collection(DomainName) <> Nothing Then
        Return CDbl(Collection(DomainName))
      Else
        Try
          Dim NullValue As String = Nothing 'Necessary for c# conversion..
          TraficRank = CLng(Common.AlexaTrafficRank(DomainName, NullValue))
          If TraficRank <> 0 Then
            ValueLink = (1000000 / (TraficRank + 4))
          End If
          ValueLink = ValueLink + 0.05
          ValueLink = Math.Round(ValueLink, 2)
          Collection.Add(DomainName, CStr(ValueLink))
          Return ValueLink
        Catch ex As Exception
  ...
  Private Sub LoadTaxtualLinkSaved()
    ...
    If Keys IsNot Nothing Then
      For n As Integer = 0 To Keys.Length - 1
        Dim TextualLink As TextualLink = CType(Persistent.LoadObject(GetType(TextualLink), Keys(n)), AdvertisingManager.TextualLink)
          ...
          If TextualLink.IsActivated Then
            ...
            Else
              ...
              For Id As Integer = 0 To TextualLink.Domains.Count - 1
                Dim Domain As String = CStr(TextualLink.Domains(Id))
                Dim Configuration As DomainConfiguration = Config.DomainConfiguration.Load(Domain)
                If Not String.IsNullOrEmpty(Configuration.Redirect) Then
  ...
  End Sub
    ...
    If PayPalAccountIsSetting() AndAlso Config.Setup.PaymentGateway.EnabledOnlinePayment Then
      ...
      For Each Domain As String In Order.Domains
        Subject &= " " & Domain
      Next
      SendEmail(Subject, ControlToText(Body), Order.Email, True, True)
    End If
	End Sub
	...
	Function LinksControl() As Control
      ...
      Dim RemoveExpired As Boolean = False
	...
	Private Sub RemoveExpired()
		For Each TextualLink As TextualLink In TextualLinkActive
			If TextualLink.IsExpired Then
				...
				SendEmailRenew(TextualLink)
        DeleteObject(GetType(TextualLink), CStr(TextualLink.ID))
				Exit For
			End If
		Next
	End Sub
