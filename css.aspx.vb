...
Partial Class css
  ...
  Protected Sub css_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Else
      If String.IsNullOrEmpty(NameSkin) Then NameSkin = NameDefault
      If IO.File.Exists(MapPath(SkinsSubDirectory & "\" & NameSkin & ".xml")) = False Then
        NameSkin = NameDefault
      End If
      Dim Skin As Config.Skin = Config.Skin.Load(NameSkin)
      ...
      For Each C As Char In CssData
        ...
        If C = " "c Then
          ...
          End If
          LastSpace = True
        ElseIf C = Microsoft.VisualBasic.ControlChars.Tab Then
        Else
          StringBuilder.Append(C)
          LastSpace = False
        End If
      ...
      For Each EachLine As String In Lines
        Dim Line As String = Trim(EachLine)
        Dim Position As Integer = InStr(Line, "/*")
        If CBool(Position) Then
          Line = WebApplication.Extension.Left(Line, Position - 1)
        End If
    ...
    If String.IsNullOrEmpty(Media) AndAlso Request.Browser.IsMobileDevice = True Then
      Media = "handheld"
    End If
    If Config.Skin.SkinUseCustomSetup.ContainsKey(SubSiteName) Then
  ...
  Sub ReplaceValues(ByRef Data As String)
    ...
    Dim SkinSetup As Config.SkinSetup = Config.SkinSetup.Load(Setups)
    ...
    End If
    Dim SortCriterion As KeyLenComparer = New KeyLenComparer
  ...
  Public Class KeyLenComparer
    ...
    Implements System.Collections.Generic.IComparer(Of css.KeyValue)
    Public Function Compare(ByVal KeyValue1 As css.KeyValue, ByVal KeyValue2 As css.KeyValue) As Integer Implements System.Collections.Generic.IComparer(Of css.KeyValue).Compare
