...
Partial Class pagesmanager
  ...
  Private Sub Page_Init(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Init
    ...
    Language = CType(ValInt(Request.QueryString("lng")), LanguageManager.Language)
		If Archive = 0 Then
          ...
          If Language = LanguageManager.Language.NotDefinite Then Language = Setting.Language
				End If
			End If
		End If
    ...
    If CurrentUser.Role(Setting.Name) < Authentication.User.RoleType.AdministratorJunior Then
      Block = True
    Else
      If CurrentUser.Role(Setting.Name) < Authentication.User.RoleType.WebMaster AndAlso Archive <> 0 Then
        Block = True
        If Setting.Archive IsNot Nothing Then
  ...
  Function Script() As Control
    ...
    "ObjTitle=document.getElementById(""" & TitleItem.ClientID & """);" & vbCrLf & _
  ...
  Sub ReplacePlaceHolder()
    PlaceHolder1.Controls.Add(IconUnicode(IconName.ArrowUp, False, Phrase(Setting.Language, 3005), "javascript:moveup()"))
    PlaceHolder2.Controls.Add(IconUnicode(IconName.ArrowDown, False, Phrase(Setting.Language, 3006), "javascript:movedown()"))
    PlaceHolder3.Controls.Add(IconUnicode(IconName.ArrowLeft, False, Phrase(Setting.Language, 3007), "javascript:declevel()"))
    PlaceHolder4.Controls.Add(IconUnicode(IconName.ArrowRight, False, Phrase(Setting.Language, 3008), "javascript:inclevel()"))
  End Sub
  ...
  Private Sub SetVisualization()
      ...
      If Setting.Archive IsNot Nothing Then
        ...
        For Each IdArchive As Integer In Setting.Archive
          If IdArchive <> 0 Then
            ...
            Dim Link As String = Href(Setting.Name, False, "pagesmanager.aspx", "archive", IdArchive, "lng", Setting.Language)
    ...
    Else
      ...
      Dim Items As Collections.Generic.List(Of MenuManager.ItemMenu) = MenuManager.Menu.Load(Archive, Language).ItemsMenu
      If Not Items Is Nothing Then
        For Each Item As MenuManager.ItemMenu In Items
  ...
  Private Sub Submit()
    ...
    WriteAll(Code, File)
    Dim MenuObj As MenuManager.Menu = MenuManager.Menu.Load(Archive, Language)
