...
Partial Class ScriptSender
  ...
  Protected Sub script_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
		...
		Dim Value As String
		Try
      Source = CType([Enum].Parse(GetType(SourceScript), Request.QueryString("s")), SourceScript)
  ...
  Private Sub SetByValue(ByVal Value As String)
    Select Case source
      Case SourceScript.DetectTimeZoneOffset
        ...
        Dim User As Authentication.User = Authentication.CurrentUser(Session)
        User.TimeOffsetSeconds = SecDifs
    End Select
    Response.End()
  End Sub
