...
#Region " Web Form Designer Generated Code "
  ...
  Private Sub Page_Init(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Init
    ...
    InitializeComponent()
    'TryAutoLogin()
    If IsLocal() = False AndAlso CurrentUser(Session).GeneralRole < Authentication.User.RoleType.Supervisor Then
      Response.End()
...
#End Region
  ...
  Public Sub RepairUser()
    ...
    For Each File As System.IO.FileSystemInfo In Files
      Dim Username As String = Extension.Left(File.Name, Len(File.Name) - 4)
      Dim VbsFile As String = Extension.Left(File.FullName, Len(File.FullName) - 4)
  ...
  Protected Sub Merge_Click(sender As Object, e As System.EventArgs) Handles Merge.Click
    ...
    While DataReader.Read
      Dim Text As String = CStr(DataReader(Lng))
      Dim ID As Integer = CInt(DataReader("ID"))
      If Update OrElse String.IsNullOrEmpty(Phrase(Language, ID)) Then
        If Phrase(Language, ID) <> Text Then
          ...
          Try
            ...
            Source.Read()
            If Phrase(LanguageManager.Language.Italian, ID) = CStr(Source("IT")) Then
              ...
              DataManager.AddRow(LocalizationDataFile, "Phrases", Cells, "ID", ID)
            End If
          Catch ex As Exception
          End Try
        End If
  ...
  Protected Sub Translate_Click(sender As Object, e As System.EventArgs) Handles Translate.Click
    Dim Language As LanguageManager.Language = LanguageManager.Language.Italian
    Dim Lng As String = Acronym(Language)

    Dim DataReader As Data.OleDb.OleDbDataReader = DataManager.Table(LocalizationDataFile, "Phrases", Nothing, "ID")
    While DataReader.Read
      Dim Cells() As DataManager.Cell = DataReaderToCells(DataReader)
      Dim Text As String = "" '= DataReader(Lng)
      Dim ID As Integer = 0 '= DataReader("ID")
      For Each Cell As DataManager.Cell In Cells
      ...
      If Not String.IsNullOrEmpty(Text) Then
        ...
        For Each TranslateTo As LanguageManager.Language In LanguageManager.LanguagesUsed
          If TranslateTo <> Language Then
            ...
            For Each Cell As DataManager.Cell In Cells
              ...
              End If
            Next
            If IsDBNull(TextTarget.Value) OrElse String.IsNullOrEmpty(CStr(TextTarget.Value)) Then
