...
Partial Class HomeUser
    ...
    If Not String.IsNullOrEmpty(Request("u")) Then
      UserProfile = Authentication.User.Load(Request("u"))
    End If
    If UserProfile Is Nothing OrElse String.IsNullOrEmpty(UserProfile.Username) Then
      EndResponse()
    End If
    ...
    Dim Action As Actions = CType(ValInt(Request.QueryString("a")), Actions)
    Dim SetValue As String = Request.QueryString("s")

    If CBool(Action) Then
      If Request.UrlReferrer Is Nothing AndAlso String.IsNullOrEmpty(CStr(Session("HomeUserReferer"))) Then
      ...
      Select Case Action
        Case Actions.ChngeRole
          ...
          Dim SetRole As Authentication.User.RoleType = CType([Enum].Parse(GetType(Authentication.User.RoleType), SetValue), Authentication.User.RoleType)
      ...
      Else
        Response.Redirect(CStr(Session("HomeUserReferer")))
      End If
    Else
	...
	Private Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
		...
		If CurrentUser.Username <> UserProfile.Username Then
          ...
          RoleList.Attributes.Add("onChange", "var NoHashLovation=location.href.split('#')[0];window.location.assign(NoHashLovation + '&a=" & Actions.ChngeRole & "&s=' + this.options[this.selectedIndex].value);")
				End If
			End If
		End If
	...
	Sub SetPanel()
		...
		If StrComp(CurrentUser.Username, UserProfile.Username, CompareMethod.Text) <> 0 Then
          ...
          Operations.Controls.Add(Components.Button(Setting, Phrase(Setting.Language, 3228), Href(Setting.Name, False, "messenger.aspx", MessengerManager.QueryStringParameters(UserProfile.Username, MessengerManager.Action.RemoveContact)), Nothing, IconType.Favourites))
				Else
					If UserProfile.Role(Setting.Name) >= Authentication.User.RoleType.User Then
						'Add button [Add this contact to favorites]
            Operations.Controls.Add(Components.Button(Setting, Phrase(Setting.Language, 3227), Href(Setting.Name, False, "messenger.aspx", MessengerManager.QueryStringParameters(UserProfile.Username, MessengerManager.Action.AddContact)), Nothing, IconType.Favourites))
					End If
				End If
			End If
		Else
			'Is current user
			'Add Button Log Off
      Operations.Controls.Add(Components.Button(Setting, Phrase(Setting.Language, 22), Href(Setting.Name, False, "log.aspx", QueryKey.ActionLog, LogActionType.LogOff), Nothing, IconType.Globe))
		End If

		If StrComp(CurrentUser.Username, UserProfile.Username, CompareMethod.Text) = 0 Then
			'Add button Modify Profile
      Operations.Controls.Add(Components.Button(Setting, Phrase(Setting.Language, 320), Href(Setting.Name, False, "log.aspx", QueryKey.ActionLog, LogActionType.EditUser), Nothing, IconType.ControlPanel))
		End If

		If CurrentUser.Role(Setting.Name) > Authentication.User.RoleType.WebMaster Then
			'Add button User Setting
      Operations.Controls.Add(Components.Button(Setting, Phrase(Setting.Language, 151), Href(Setting.Name, False, "profilemanager.aspx", "u", UserProfile.Username), Nothing, IconType.ControlPanel))
		End If
    ...
    Dim BtnMessageBoard As Control = Components.Button(Setting, Phrase(Setting.Language, 121), HrefMessageBoard, Nothing, IconType.Contacts)
	...
	Sub PopulateInfoPanel()
      ...
      AddInfo(Phrase(Setting.Language, 112), UserProfile.FirstAccess.ToString(Setting.DateFormat, Setting.Culture()), Nothing, Nothing, "name")
		End If
    If Not String.IsNullOrEmpty(UserProfile.URL) Then
      ...
      If PageAuthor.Contains("://plus.google.com/") Then
        ...
        End If
      End If
      AddInfo(Phrase(Setting.Language, 18), UserProfile.URL, Nothing, PageAuthor, "url", "author")
    End If
    AddInfo(Phrase(Setting.Language, 113), UserProfile.LastAccess.ToString(Setting.DateFormat, Setting.Culture()))
    If Extension.IsCrawler(Request) OrElse CurrentUser.Role(Setting.Name) >= Authentication.User.RoleType.User OrElse CurrentUser.Username = UserProfile.Username Then

      AddInfo(Phrase(Setting.Language, 24), UserProfile.FirstName, Nothing, Nothing, "givenName")
      AddInfo(Phrase(Setting.Language, 25), UserProfile.LastName, Nothing, Nothing, "familyName")
      If UserProfile.Gender <> Authentication.User.GenderType.NotDefinite Then
        ...
        End If
        AddInfo(Phrase(Setting.Language, 19), Gender, Nothing, Nothing, "gender")
      End If
      If Not String.IsNullOrEmpty(UserProfile.Phone) Then
        AddInfo(Phrase(Setting.Language, 15), UserProfile.Phone, Nothing, "callto://" & UserProfile.Phone, "telephone", "nofollow")
      End If
      ...
      Dim RatingFromUser As Integer = 0
      ...
      ratingValue.Attributes.Add("content", CInt(UserProfile.Rating).ToString())
      ...
      ratingValue.Attributes.Add("content", CStr(5))
      ...
      If Not String.IsNullOrEmpty(CurrentUser.Username) Then
        For N As Integer = 1 To 5
          ...
          If CurrentUser.Username <> UserProfile.Username Then
            LinkUrl = Href(Setting.Name, False, "messenger.aspx", MessengerManager.QueryStringParameters(UserProfile.Username, MessengerManager.Action.RatingUser, False, CStr(N)))
          End If
        ...
        Next
      End If
      AddInfo(Phrase(Setting.Language, 433), Nothing, RatingControl)
    End If
        ...
        LockUser = IconUnicode(IconName.Block, False, Phrase(Setting.Language, 3193))
				Lock.Controls.Add(LockUser)
			ElseIf CurrentUser.GeneralRole > UserProfile.GeneralRole Then
				'Add tool to block this code user
        Dim NavigateUrl As String = Href(Setting.Name, False, "messenger.aspx", MessengerManager.QueryStringParameters(UserProfile.Username, MessengerManager.Action.BlockUser))
        LockUser = IconUnicode(IconName.ScissorBlack, False, Phrase(Setting.Language, 3233), NavigateUrl)
				Lock.Controls.Add(LockUser)
			End If
      Dim LockIp As Control = Nothing
			If IPIsBlocked(UserProfile.IP) Then
				'This code user is blocked
        LockIp = IconUnicode(IconName.Block, False, Phrase(Setting.Language, 3193), Nothing)
			End If
			If UserProfile.Rating <> 0 Then
				Dim Culture As Globalization.CultureInfo = New Globalization.CultureInfo("en-US")
        AddInfo("Rating", UserProfile.Rating.ToString("0.##", Culture))
			End If
			...
			End If
      If Not String.IsNullOrEmpty(UserProfile.Forwarded) Then
        AddInfo("Forwarded", UserProfile.Forwarded)
      End If
      If Not String.IsNullOrEmpty(UserProfile.Identity) Then
        AddInfo("Identity", UserProfile.Identity)
      End If
      AddInfo(Phrase(Setting.Language, 12), UserProfile.Email, Nothing, "mailto:" & UserProfile.Email, "email") 'E-Mail
      If Not String.IsNullOrEmpty(UserProfile.Generate) Then
      ...
      AddInfo("Time offset (sec.)", CStr(UserProfile.TimeOffsetSeconds))

      If Not String.IsNullOrEmpty(UserProfile.CodeUser) Then
        ...
        If CodesBanned IsNot Nothing AndAlso CodesBanned.Contains(UserProfile.CodeUser) Then
          'This code user is blocked
          LockCode = IconUnicode(IconName.Block, False, Phrase(Setting.Language, 3193), Nothing)
        End If
        AddInfo(Phrase(Setting.Language, 3234), UserProfile.CodeUser, LockCode)
      End If
		End If

    If CBool(InfoPanel.Controls.Count) Then
    ...
    If Not PhotoAlbum Is Nothing Then
      ...
      Dim Button As Control = Components.Button(Setting, Phrase(Setting.Language, 3224), Href(Setting.Name, False, "thumbnails.aspx", QueryKey.ViewAlbum, PhotoAlbum.Name), Nothing, IconType.FolderPics1)
      Operations.Controls.Add(Button)
      AddFotoalbumSlideShow(Setting, Button, PhotoAlbum.Name)
      Dim AllPhotosName As Integer() = PhotoManager.AllPhotosName(PhotoAlbum.Name)
      If Not AllPhotosName Is Nothing AndAlso AllPhotosName.Length > 0 Then
        ...
        If Extension.IsCrawler(Request) Then
          AddInfo(Phrase(Setting.Language, 104), FirsPhoto.Src(Setting), Nothing, FirsPhoto.Src(Setting), "image")
        End If

        PhotosPanel.Controls.Add(FirsPhoto.ControlPhoto(Setting))
        Dim Count As Integer = 0
        Dim MiniPhotos As New Control
        For Each PhotoName As Integer In AllPhotosName
          If PhotoName <> FirsPhoto.Name Then
            Count += 1
            Dim Photo As Photo = PhotoManager.Photo.Load(PhotoName, PhotoAlbum.Name)
            MiniPhotos.Controls.Add(Photo.ControlThumbnail(Setting, True, 0, ThumbnailBoxMode.Gallery))
          End If
          If Count = 5 Then Exit For
        Next
        If CBool(MiniPhotos.Controls.Count) Then
